# Sample: Kernel Nsys Profiling - CUDA C++ Kernel Profiling with cuda.core (Python) - Japanese Learning Guide
...
The program uses color-coded NVTX markers: - **Purple**: Phase 2 (cuda.core Custom Kernels - main focus) - **Yellow/Blue/Green**: Other phases - **Cyan**: Nested operations

Original README headings: `Sample: Kernel Nsys Profiling - CUDA C++ Kernel Profiling with cuda.core (Python)`, `Description`, `What you will learn`, `Requirements`, `How to run`, `Nsys Profiling`, `Troubleshooting`, `See Also`

> **日本語**
> `python/1_GettingStarted/kernelNsysProfile` は `Python` の CUDA sample です。英語 README を正本として残し、この guide では実装を読む順番と CUDA の学習ポイントを日本語で補足します。
>
> **学習メモ**
...
English anchor: read `kernelNsysProfile` as a focused example of the CUDA concepts used in `python/1_GettingStarted/kernelNsysProfile`.
> **日本語**
> この sample の目的は、`kernelNsysProfile` の小さな実装を通して Python CUDA, Runtime, Driver, And NVRTC, Multi-GPU, P2P, And IPC, Streams And Events, Performance を具体的に追うことです。
>
> **学習メモ**
> 最初に `kernelNsysProfile.py` を読み、setup、GPU work、sync、validation、cleanup の境界を source 内で対応させます。
...
- `kernelNsysProfile.py`: Python entry point or helper using CUDA Python, CuPy, framework interop, or subprocess logic.
...
- Open `kernelNsysProfile.py` first and locate the host-side setup or Python entry point.
...
- `kernelNsysProfile.py`: focus on `CUDA`, `launch`, `Device`, `blockDim`, `threadIdx`.
...
| `cp.std` | Python object から CUDA resource や device work を扱う境界です。hidden sync に注意します。 |
