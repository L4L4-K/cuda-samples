# Sample: Image Blur with Unified Memory (Python) - Japanese Learning Guide
...
Original README headings: `Sample: Image Blur with Unified Memory (Python)`, `Description`, `What You'll Learn`, `Key Concepts`, `Kernel Compilation with cuda.core.Program`, `Compile CUDA C++ kernel at runtime`, `Kernel Launch with cuda.core.launch`, `Configure and launch kernel`, `Buffers can be passed directly as kernel arguments`, `Unified Memory (Managed Memory)`, `Allocate unified memory`, `Synchronize to ensure allocations are complete before CPU access`

> **日本語**
> `python/1_GettingStarted/blurImageUnifiedMemory` は `Python` の CUDA sample です。英語 README を正本として残し、この guide では実装を読む順番と CUDA の学習ポイントを日本語で補足します。
>
> **学習メモ**
...
English anchor: read `blurImageUnifiedMemory` as a focused example of the CUDA concepts used in `python/1_GettingStarted/blurImageUnifiedMemory`.
> **日本語**
> この sample の目的は、`blurImageUnifiedMemory` の小さな実装を通して Python CUDA, Runtime, Driver, And NVRTC, Multi-GPU, P2P, And IPC, Streams And Events, Unified Memory を具体的に追うことです。
>
> **学習メモ**
> 最初に `blurImageUnifiedMemory.py` を読み、setup、GPU work、sync、validation、cleanup の境界を source 内で対応させます。
...
- `blurImageUnifiedMemory.py`: Python entry point or helper using CUDA Python, CuPy, framework interop, or subprocess logic.
...
- Open `blurImageUnifiedMemory.py` first and locate the host-side setup or Python entry point.
...
- `blurImageUnifiedMemory.py`: focus on `launch`, `CUDA`, `Program`, `ManagedMemoryResource`, `Device`.
