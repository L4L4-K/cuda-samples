# Image Blur with Unified Memory (Python) - Japanese Learning Guide
...
Original README headings: `Sample: Image Blur with Unified Memory (Python)`, `Description`, `What You'll Learn`, `Key Concepts`, `Kernel Compilation with cuda.core.Program`, `Compile CUDA C++ kernel at runtime`, `Kernel Launch with cuda.core.launch`, `Configure and launch kernel`

> **日本語**
> `python/1_GettingStarted/blurImageUnifiedMemory` は `1_GettingStarted` に属する CUDA sample です。英語 README の目的と手順を残し、ここでは日本語で実装の読み方を補足します。
>
> **学習メモ**
...
English anchor: this sample demonstrates `Image Blur with Unified Memory (Python)` and should be read together with the original README and source files.
