# Image Array Copy to GPU (Python) - Japanese Learning Guide
...
Original README headings: `Sample: Image Array Copy to GPU (Python)`, `Description`, `What You'll Learn`, `Key Libraries`, `Key APIs`, `From `cuda.core`:`, `From `numpy`:`, `From `cupy`:`

> **日本語**
> `python/1_GettingStarted/copyImageArraytoGPU` は `1_GettingStarted` に属する CUDA sample です。英語 README の目的と手順を残し、ここでは日本語で実装の読み方を補足します。
>
> **学習メモ**
...
English anchor: this sample demonstrates `Image Array Copy to GPU (Python)` and should be read together with the original README and source files.
