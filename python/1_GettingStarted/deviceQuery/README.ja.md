# Device Query (Python) - Japanese Learning Guide
...
Original README headings: `Sample: Device Query (Python)`, `Description`, `What You'll Learn`, `Key Libraries`, `Key APIs`, `From `cuda.core`:`, `From `cuda.bindings.runtime`:`, `From `cuda.bindings.driver`:`

> **日本語**
> `python/1_GettingStarted/deviceQuery` は `1_GettingStarted` に属する CUDA sample です。英語 README の目的と手順を残し、ここでは日本語で実装の読み方を補足します。
>
> **学習メモ**
...
English anchor: this sample demonstrates `Device Query (Python)` and should be read together with the original README and source files.
