# Launch Configuration Tuning (Python) - Japanese Learning Guide
...
Original README headings: `Sample: Launch Configuration Tuning (Python)`, `Description`, `What You'll Learn`, `Key Concepts`, `Launch Configuration with cuda.core`, `Configure kernel launch with specific thread block size`, `Launch kernel`, `Thread Block Sizing`

> **日本語**
> `python/2_CoreConcepts/launchConfigTuning` は `2_CoreConcepts` に属する CUDA sample です。英語 README の目的と手順を残し、ここでは日本語で実装の読み方を補足します。
>
> **学習メモ**
...
English anchor: this sample demonstrates `Launch Configuration Tuning (Python)` and should be read together with the original README and source files.
