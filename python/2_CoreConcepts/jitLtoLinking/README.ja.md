# JIT Compilation and Link-Time Optimization (Python) - Japanese Learning Guide
...
Original README headings: `JIT Compilation and Link-Time Optimization (Python)`, `Description`, `What You'll Learn`, `Key Libraries`, `Key APIs`, `From `cuda.core``, `From `cuda_samples_utils``, `Requirements`, `Hardware`, `Software`, `Installation`, `How to Run`

> **日本語**
> `python/2_CoreConcepts/jitLtoLinking` は `Python` の CUDA sample です。英語 README を正本として残し、この guide では実装を読む順番と CUDA の学習ポイントを日本語で補足します。
>
> **学習メモ**
...
English anchor: read `jitLtoLinking` as a focused example of the CUDA concepts used in `python/2_CoreConcepts/jitLtoLinking`.
> **日本語**
> この sample の目的は、`jitLtoLinking` の小さな実装を通して Python CUDA, Runtime, Driver, And NVRTC, Multi-GPU, P2P, And IPC, Streams And Events, Synchronization And Atomics を具体的に追うことです。
>
> **学習メモ**
> 最初に `jitLtoLinking.py` を読み、setup、GPU work、sync、validation、cleanup の境界を source 内で対応させます。
...
- `jitLtoLinking.py`: Python entry point or helper using CUDA Python, CuPy, framework interop, or subprocess logic.
...
- Open `jitLtoLinking.py` first and locate the host-side setup or Python entry point.
...
- `Program` の直前と直後で、どの memory/resource が有効になったかをメモする。
