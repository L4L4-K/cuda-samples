...
This sample demonstrates how to use the **CUDA process checkpoint API** via `cuda.core.checkpoint.Process` to suspend, capture, and restore the CUDA state of a running Linux process. CUDA process checkpointing is the driver-level primitive that powers CRIU + `cuda-checkpoint` integration. The sample: 1. Allocates a GPU buffer and fills it with a deterministic pattern

Original README headings: `processCheckpoint (Python)`, `Description`, `What You'll Learn`, `Key Libraries`, `Key APIs`, `From `cuda.core.checkpoint``, `From `cuda.core``, `From `cuda.bindings.driver``

> **日本語**
> `python/2_CoreConcepts/processCheckpoint` は `2_CoreConcepts` に属する CUDA sample です。英語 README の目的と手順を残し、ここでは日本語で実装の読み方を補足します。
>
> **学習メモ**
...
English anchor: this sample demonstrates `processCheckpoint (Python)` and should be read together with the original README and source files.
