...
This sample demonstrates how to capture a multi-stage kernel pipeline as a CUDA graph with `cuda.core` and replay it with a single driver call. The sample runs a three-stage elementwise pipeline `r3 = (a + b) * c - a` in two modes: 1. **Individual launches** - one `launch(stream, ...)` per stage, repeated for every iteration of the pipeline. 2. **CUDA graph replay** - the same three launches are recorded into a

Original README headings: `cudaGraphs (Python)`, `Description`, `What You'll Learn`, `Key Libraries`, `Key APIs`, `From `cuda.core``, `From `cuda_samples_utils``, `Requirements`

> **日本語**
> `python/2_CoreConcepts/cudaGraphs` は `2_CoreConcepts` に属する CUDA sample です。英語 README の目的と手順を残し、ここでは日本語で実装の読み方を補足します。
>
> **学習メモ**
...
English anchor: this sample demonstrates `cudaGraphs (Python)` and should be read together with the original README and source files.
