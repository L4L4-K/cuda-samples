# Memory Resources and Buffers (Python) - Japanese Learning Guide
...
This sample demonstrates the `cuda.core` memory management model: a `MemoryResource` owns a pool of memory and hands out `Buffer` objects that can be passed to kernels, copied between resources with `Buffer.copy_to()`, and viewed as NumPy or CuPy arrays through DLPack. The script exercises three common resources side-by-side: 1. **`DeviceMemoryResource`** - device-local GPU memory. Every `Device`

Original README headings: `Sample: Memory Resources and Buffers (Python)`, `Description`, `What You'll Learn`, `Key Libraries`, `Key APIs`, `From `cuda.core``, `From CuPy and NumPy`, `From `cuda_samples_utils``

> **日本語**
> `python/2_CoreConcepts/memoryResources` は `2_CoreConcepts` に属する CUDA sample です。英語 README の目的と手順を残し、ここでは日本語で実装の読み方を補足します。
>
> **学習メモ**
...
English anchor: this sample demonstrates `Memory Resources and Buffers (Python)` and should be read together with the original README and source files.
