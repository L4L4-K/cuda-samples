# FFT Signal Analysis (Python) - Japanese Learning Guide
...
Original README headings: `Sample: FFT Signal Analysis (Python)`, `Description`, `What You'll Learn`, `Key Concepts`, `Stream Interop`, `Create stream with cuda.core`, `Use with CuPy operations`, `Key APIs`

> **日本語**
> `python/2_CoreConcepts/fftSignalAnalysis` は `2_CoreConcepts` に属する CUDA sample です。英語 README の目的と手順を残し、ここでは日本語で実装の読み方を補足します。
>
> **学習メモ**
...
English anchor: this sample demonstrates `FFT Signal Analysis (Python)` and should be read together with the original README and source files.
