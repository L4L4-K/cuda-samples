# Streaming Copy + Compute Overlap (Python) - Japanese Learning Guide
...
Original README headings: `Sample: Streaming Copy + Compute Overlap (Python)`, `Description`, `What You'll Learn`, `Key Concept`, `Key APIs (all from `cuda.core`)`, `From `numpy`:`, `Requirements`, `Installation`

> **日本語**
> `python/2_CoreConcepts/streamingCopyComputeOverlap` は `2_CoreConcepts` に属する CUDA sample です。英語 README の目的と手順を残し、ここでは日本語で実装の読み方を補足します。
>
> **学習メモ**
...
English anchor: this sample demonstrates `Streaming Copy + Compute Overlap (Python)` and should be read together with the original README and source files.
