# Sample: multiGPUGradientAverage (Python) - Japanese Learning Guide
...
least 2 processes. On Linux/macOS this is typically `mpirun`; on Windows with Microsoft MPI the launcher is `mpiexec` (and the flag for process count is `-n`). Either form is accepted by most MPI stacks.

Original README headings: `Sample: multiGPUGradientAverage (Python)`, `Description`, `What you will learn`, `Prerequisites`, `Installation`, `Running`, `Single node (2 GPUs)`, `Single node (4 GPUs)`, `With specific GPUs`, `Sample Output`, `Key Technical Details`, `... GPU work ...`

> **日本語**
> `python/4_DistributedComputing/multiGPUGradientAverage` は `Python` の CUDA sample です。英語 README を正本として残し、この guide では実装を読む順番と CUDA の学習ポイントを日本語で補足します。
>
> **学習メモ**
...
English anchor: read `multiGPUGradientAverage` as a focused example of the CUDA concepts used in `python/4_DistributedComputing/multiGPUGradientAverage`.
> **日本語**
> この sample の目的は、`multiGPUGradientAverage` の小さな実装を通して Python CUDA, Runtime, Driver, And NVRTC, Multi-GPU, P2P, And IPC, Streams And Events, Synchronization And Atomics を具体的に追うことです。
>
> **学習メモ**
> 最初に `multiGPUGradientAverage.py` を読み、setup、GPU work、sync、validation、cleanup の境界を source 内で対応させます。
...
- `multiGPUGradientAverage.py`: Python entry point or helper using CUDA Python, CuPy, framework interop, or subprocess logic.
...
- Open `multiGPUGradientAverage.py` first and locate the host-side setup or Python entry point.
...
- `multiGPUGradientAverage.py`: focus on `CUDA`, `Device`, `launch`, `Stream`, `Event`.
...
| `mpi4py` | この sample の中心 API/概念です。入力、所有権、同期、検証との関係を確認します。 |
...
| `CUDA_VISIBLE_DEVICES` | この sample の中心 API/概念です。入力、所有権、同期、検証との関係を確認します。 |
...
python multiGPUGradientAverage.py
```
