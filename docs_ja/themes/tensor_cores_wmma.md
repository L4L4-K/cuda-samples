...
English anchor: Tensor Core samples use specialized matrix instructions, tile APIs, and precision formats for high-throughput math.

> **日本語**
> Tensor Core/WMMA sample は、通常の scalar multiply-add ではなく warp/tile 単位の matrix operation を specialized hardware に渡します。precision、layout、alignment、tile size、accumulator type が重要です。
>
> **学習メモ**
...
- Tensor Cores And WMMA は correctness、resource lifetime、performance の読み方に直接関係します。
...
- Tensor Cores/WMMA では、どの thread/process/API が resource を所有するかを先に分けます。
- Tensor Cores/WMMA の API は便利な wrapper に見えても、lifetime、ordering、visibility の境界を持ちます。
...
| - | - | - |
| `wmma::fragment` | WMMA tile fragment です。 | matrix_a、matrix_b、accumulator と layout を確認します。 |
| `wmma::load_matrix_sync / mma_sync / store_matrix_sync` | tile load、multiply、store です。 | leading dimension と precision を確認します。 |
| `cublasSetMathMode / cuBLASLt` | library 経由の Tensor Core path です。 | math mode、compute type、workspace を確認します。 |
| `CUDA Tile APIs` | tile-level programming model です。 | tile shape と backend selection を確認します。 |
...
- [cudaTensorCoreGemm](../../cpp/3_CUDA_Features/cudaTensorCoreGemm/README.ja.md): WMMA/Tensor Core GEMM を読みます。
- [tf32TensorCoreGemm](../../cpp/3_CUDA_Features/tf32TensorCoreGemm/README.ja.md): TF32 を読みます。
- [bf16TensorCoreGemm](../../cpp/3_CUDA_Features/bf16TensorCoreGemm/README.ja.md): BF16 を読みます。
- [immaTensorCoreGemm](../../cpp/3_CUDA_Features/immaTensorCoreGemm/README.ja.md): INT matrix path を読みます。
- [tileMatmul](../../cpp/9_CUDA_Tile/tileMatmul/README.ja.md): CUDA Tile matmul を読みます。

## Representative Code

### `cpp/3_CUDA_Features/cudaTensorCoreGemm/CMakeLists.txt`

Source: cpp/3_CUDA_Features/cudaTensorCoreGemm/CMakeLists.txt:1-38
```cmake
...
> JP: この抜粋は `cpp/3_CUDA_Features/cudaTensorCoreGemm/CMakeLists.txt` の実コードです。setup、allocation、transfer、GPU work、sync、validation、cleanup のどの境界を示すかを、行番号と一緒に確認します。

> JP: `cpp/3_CUDA_Features/cudaTensorCoreGemm/CMakeLists.txt` はこのテーマを読むための代表例です。API 名だけでなく、所有権、同期位置、検証位置を抜粋内で対応付けます。

### `cpp/3_CUDA_Features/tf32TensorCoreGemm/CMakeLists.txt`
...
> JP: `cpp/3_CUDA_Features/tf32TensorCoreGemm/CMakeLists.txt` はこのテーマを読むための代表例です。API 名だけでなく、所有権、同期位置、検証位置を抜粋内で対応付けます。

### `cpp/3_CUDA_Features/bf16TensorCoreGemm/CMakeLists.txt`

Source: cpp/3_CUDA_Features/bf16TensorCoreGemm/CMakeLists.txt:1-42
```cmake
# JP: この build file では CMake target、CUDA architecture、library dependency を確認します。target 名や link 設定は英語のまま保持します。
...
> JP: この抜粋は `cpp/3_CUDA_Features/bf16TensorCoreGemm/CMakeLists.txt` の実コードです。setup、allocation、transfer、GPU work、sync、validation、cleanup のどの境界を示すかを、行番号と一緒に確認します。

> JP: `cpp/3_CUDA_Features/bf16TensorCoreGemm/CMakeLists.txt` はこのテーマを読むための代表例です。API 名だけでなく、所有権、同期位置、検証位置を抜粋内で対応付けます。


## Reading Steps

1. Tensor Cores/WMMA に関係する API call を探し、作成、設定、利用、破棄を対応させます。
...
- Tensor Cores/WMMA の scope を大きく見積もり、実際には保証されない ordering を期待する。
...
- Tensor Cores/WMMA は correctness の仕組みであると同時に overhead や bottleneck になり得ます。
- timing では setup、GPU work、transfer、sync、validation を分けます。
- architecture、driver、problem size に依存するため、sample の数値は相対比較として読みます。

## Exercises

- Tensor Cores/WMMA に関係する API を 5 つ選び、owner、input、output、cleanup を表にする。
- sample の timeline を setup、GPU work、sync、validation、cleanup に分ける。
- 関連 theme を 1 つ読み、同じ API pattern がどこで再利用されているか探す。

## Cross-Theme Links

- [Shared Memory](shared_memory.md): tile staging を読みます。
- [Libraries](libraries.md): cuBLAS/cuBLASLt path を読みます。
- [Performance](performance.md): throughput と memory bottleneck を読みます。
