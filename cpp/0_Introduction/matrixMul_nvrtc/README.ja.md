...
Original README headings: `matrixMul_nvrtc - Matrix Multiplication with libNVRTC`, `Description`, `Key Concepts`, `Supported SM Architectures`, `Supported OSes`, `Supported CPU Architecture`, `CUDA APIs involved`, `[CUDA Driver API](http://docs.nvidia.com/cuda/cuda-driver-api/index.html)`

> **日本語**
> `cpp/0_Introduction/matrixMul_nvrtc` は `0_Introduction` に属する CUDA sample です。英語 README の目的と手順を残し、ここでは日本語で実装の読み方を補足します。
>
> **学習メモ**
...
English anchor: this sample demonstrates `matrixMul_nvrtc - Matrix Multiplication with libNVRTC` and should be read together with the original README and source files.
