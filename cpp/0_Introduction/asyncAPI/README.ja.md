# asyncAPI - asyncAPI - Japanese Learning Guide
...
Original README headings: `asyncAPI - asyncAPI`, `Description`, `Key Concepts`, `Supported SM Architectures`, `Supported OSes`, `Supported CPU Architecture`, `CUDA APIs involved`, `[CUDA Runtime API](http://docs.nvidia.com/cuda/cuda-runtime-api/index.html)`, `Prerequisites`, `References (for more details)`

> **日本語**
> `cpp/0_Introduction/asyncAPI` は `C++/CUDA` の CUDA sample です。英語 README を正本として残し、この guide では実装を読む順番と CUDA の学習ポイントを日本語で補足します。
>
> **学習メモ**
...
English anchor: read `asyncAPI` as a focused example of the CUDA concepts used in `cpp/0_Introduction/asyncAPI`.
> **日本語**
> この sample の目的は、`asyncAPI` の小さな実装を通して Streams And Events, Performance, Memory, Kernel Launch And Indexing, Execution Model を具体的に追うことです。
>
> **学習メモ**
> 最初に `asyncAPI.cu` を読み、setup、GPU work、sync、validation、cleanup の境界を source 内で対応させます。
...
- `asyncAPI.cu`: CUDA source containing kernels, Runtime API calls, or device-side helper code.
...
- Open `asyncAPI.cu` first and locate the host-side setup or Python entry point.
...
- `asyncAPI.cu`: focus on `CUDA`, `launch`, `cudaEventDestroy`, `blockIdx`, `blockDim`.
...
Source: cpp/0_Introduction/asyncAPI/asyncAPI.cu:39-145
```cuda
#include <stdio.h>
...
> JP: この抜粋は `cpp/0_Introduction/asyncAPI/asyncAPI.cu` の実コードです。setup、allocation、transfer、GPU work、sync、validation、cleanup のどの境界を示すかを、行番号と一緒に確認します。
...
| `cudaProfilerStart` | この sample の中心 API/概念です。入力、所有権、同期、検証との関係を確認します。 |
| `cudaProfilerStop` | この sample の中心 API/概念です。入力、所有権、同期、検証との関係を確認します。 |
| `cudaEventQuery` | 非同期 work の順序、overlap、計測範囲を表す API です。 |
...
```bash
cmake -S . -B build
cmake --build build --target asyncAPI
ctest --test-dir build -R asyncAPI
```
