# simpleAtomicIntrinsics_nvrtc - Simple Atomic Intrinsics with libNVRTC - Japanese Learning Guide
...
English anchor: read `simpleAtomicIntrinsics_nvrtc` as a focused example of the CUDA concepts used in `cpp/0_Introduction/simpleAtomicIntrinsics_nvrtc`.
> **日本語**
> この sample の目的は、`simpleAtomicIntrinsics_nvrtc` の小さな実装を通して Runtime, Driver, And NVRTC, Multi-GPU, P2P, And IPC, Streams And Events, Synchronization And Atomics, Memory を具体的に追うことです。
>
> **学習メモ**
> 最初に `simpleAtomicIntrinsics.cpp, simpleAtomicIntrinsics_cpu.cpp, simpleAtomicIntrinsics_kernel.cuh` を読み、setup、GPU work、sync、validation、cleanup の境界を source 内で対応させます。
...
- `simpleAtomicIntrinsics.cpp`: Host-side setup, API calls, validation, and cleanup.
- `simpleAtomicIntrinsics_cpu.cpp`: Host-side setup, API calls, validation, and cleanup.
- `simpleAtomicIntrinsics_kernel.cuh`: CUDA header with device functions, kernels, templates, or shared constants.
...
- Open `simpleAtomicIntrinsics.cpp` first and locate the host-side setup or Python entry point.
...
- `simpleAtomicIntrinsics.cpp`: focus on `cudaBlockSize`, `cudaGridSize`, `atomic`, `CUDA`, `cuMemAlloc`.
- `simpleAtomicIntrinsics_cpu.cpp`: focus on `atomic`, `atomicAdd`, `atomicSub`, `atomicExch`, `atomicMax`.
- `simpleAtomicIntrinsics_kernel.cuh`: focus on `atomic`, `blockDim`, `blockIdx`, `threadIdx`, `launch`.
...
Source: cpp/0_Introduction/simpleAtomicIntrinsics_nvrtc/CMakeLists.txt:1-48
```cmake
...
> JP: この抜粋は `cpp/0_Introduction/simpleAtomicIntrinsics_nvrtc/CMakeLists.txt` の実コードです。setup、allocation、transfer、GPU work、sync、validation、cleanup のどの境界を示すかを、行番号と一緒に確認します。

### `simpleAtomicIntrinsics.cpp`

Source: cpp/0_Introduction/simpleAtomicIntrinsics_nvrtc/simpleAtomicIntrinsics.cpp:34-52
```cpp
#include <math.h>
#include <stdio.h>
#include <stdlib.h>
...
> JP: この抜粋は `cpp/0_Introduction/simpleAtomicIntrinsics_nvrtc/simpleAtomicIntrinsics.cpp` の実コードです。setup、allocation、transfer、GPU work、sync、validation、cleanup のどの境界を示すかを、行番号と一緒に確認します。

Source: cpp/0_Introduction/simpleAtomicIntrinsics_nvrtc/simpleAtomicIntrinsics.cpp:63-82
```cpp
...
Source: cpp/0_Introduction/simpleAtomicIntrinsics_nvrtc/simpleAtomicIntrinsics.cpp:104-144
```cpp
...
Source: cpp/0_Introduction/simpleAtomicIntrinsics_nvrtc/simpleAtomicIntrinsics_cpu.cpp:29-47
```cpp
#include <math.h>
#include <stdio.h>
...
> JP: この抜粋は `cpp/0_Introduction/simpleAtomicIntrinsics_nvrtc/simpleAtomicIntrinsics_cpu.cpp` の実コードです。setup、allocation、transfer、GPU work、sync、validation、cleanup のどの境界を示すかを、行番号と一緒に確認します。

Source: cpp/0_Introduction/simpleAtomicIntrinsics_nvrtc/simpleAtomicIntrinsics_cpu.cpp:51-70
```cpp
    for (int i = 0; i < len; ++i) {
        val += 10;
    }
...
Source: cpp/0_Introduction/simpleAtomicIntrinsics_nvrtc/simpleAtomicIntrinsics_kernel.cuh:31-83
```cuda
...
> JP: この抜粋は `cpp/0_Introduction/simpleAtomicIntrinsics_nvrtc/simpleAtomicIntrinsics_kernel.cuh` の実コードです。setup、allocation、transfer、GPU work、sync、validation、cleanup のどの境界を示すかを、行番号と一緒に確認します。
...
| `atomicSub` | 複数 thread が同じ address を更新する箇所です。競合と順序の意味を確認します。 |
...
```bash
cmake -S . -B build
cmake --build build --target simpleAtomicIntrinsics_nvrtc
ctest --test-dir build -R simpleAtomicIntrinsics_nvrtc
```
...
- `atomic` の直前と直後で、どの memory/resource が有効になったかをメモする。
