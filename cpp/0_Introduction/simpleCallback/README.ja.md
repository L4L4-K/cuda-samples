# simpleCallback - Simple CUDA Callbacks - Japanese Learning Guide
...
Original README headings: `simpleCallback - Simple CUDA Callbacks`, `Description`, `Key Concepts`, `Supported SM Architectures`, `Supported OSes`, `Supported CPU Architecture`, `CUDA APIs involved`, `[CUDA Runtime API](http://docs.nvidia.com/cuda/cuda-runtime-api/index.html)`, `Prerequisites`, `References (for more details)`

> **日本語**
> `cpp/0_Introduction/simpleCallback` は `C++/CUDA` の CUDA sample です。英語 README を正本として残し、この guide では実装を読む順番と CUDA の学習ポイントを日本語で補足します。
>
> **学習メモ**
...
English anchor: read `simpleCallback` as a focused example of the CUDA concepts used in `cpp/0_Introduction/simpleCallback`.
> **日本語**
> この sample の目的は、`simpleCallback` の小さな実装を通して Streams And Events, Synchronization And Atomics, Memory, Kernel Launch And Indexing, Execution Model を具体的に追うことです。
>
> **学習メモ**
> 最初に `multithreading.cpp, multithreading.h, simpleCallback.cu` を読み、setup、GPU work、sync、validation、cleanup の境界を source 内で対応させます。
...
- `simpleCallback.cu`: CUDA source containing kernels, Runtime API calls, or device-side helper code.
...
- Open `multithreading.cpp` first and locate the host-side setup or Python entry point.
...
- `multithreading.cpp`: focus on `CUTBarrier`, `CUTThread`, `CUT_THREADROUTINE`, `atomic`.
- `multithreading.h`: focus on `CUTBarrier`, `CUTThread`, `CUT_THREADROUTINE`, `CUT_THREADPROC`, `CUT_THREADEND`.
- `simpleCallback.cu`: focus on `CUDA`, `launch`, `cudaStream_t`, `cudaDeviceID`, `cudaError_t`.
...
| - | - |
| `CUTBarrier` | Driver API の handle 境界です。context/module/function と error code を追います。 |
...
| `CUT_THREADPROC` | Driver API の handle 境界です。context/module/function と error code を追います。 |
| `CUT_THREADEND` | Driver API の handle 境界です。context/module/function と error code を追います。 |
...
```bash
cmake -S . -B build
cmake --build build --target simpleCallback
ctest --test-dir build -R simpleCallback
```
...
- `CUTBarrier` の直前と直後で、どの memory/resource が有効になったかをメモする。
