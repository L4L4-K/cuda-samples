# vectorAdd - Vector Addition - Japanese Learning Guide
...
Original README headings: `vectorAdd - Vector Addition`, `Description`, `Key Concepts`, `Supported SM Architectures`, `Supported OSes`, `Supported CPU Architecture`, `CUDA APIs involved`, `[CUDA Runtime API](http://docs.nvidia.com/cuda/cuda-runtime-api/index.html)`, `Prerequisites`, `References (for more details)`

> **日本語**
> `cpp/0_Introduction/vectorAdd` は `C++/CUDA` の CUDA sample です。英語 README を正本として残し、この guide では実装を読む順番と CUDA の学習ポイントを日本語で補足します。
>
> **学習メモ**
...
English anchor: read `vectorAdd` as a focused example of the CUDA concepts used in `cpp/0_Introduction/vectorAdd`.
> **日本語**
> この sample の目的は、`vectorAdd` の小さな実装を通して Memory, Kernel Launch And Indexing, Execution Model, Debugging, Profiling, And Testing を具体的に追うことです。
>
> **学習メモ**
> 最初に `vectorAdd.cu` を読み、setup、GPU work、sync、validation、cleanup の境界を source 内で対応させます。
...
- `vectorAdd.cu`: CUDA source containing kernels, Runtime API calls, or device-side helper code.
...
- Open `vectorAdd.cu` first and locate the host-side setup or Python entry point.
...
- `vectorAdd.cu`: focus on `cudaSuccess`, `cudaGetErrorString`, `CUDA`, `launch`, `cudaMalloc`.
...
Source: cpp/0_Introduction/vectorAdd/vectorAdd.cu:37-86
```cuda
#include <stdio.h>
...
> JP: この抜粋は `cpp/0_Introduction/vectorAdd/vectorAdd.cu` の実コードです。setup、allocation、transfer、GPU work、sync、validation、cleanup のどの境界を示すかを、行番号と一緒に確認します。

Source: cpp/0_Introduction/vectorAdd/vectorAdd.cu:121-140
```cuda
...
Source: cpp/0_Introduction/vectorAdd/vectorAdd.cu:171-190
```cuda
    }
...
```bash
cmake -S . -B build
cmake --build build --target vectorAdd
ctest --test-dir build -R vectorAdd
```
...
- `cudaGetErrorString` の直前と直後で、どの memory/resource が有効になったかをメモする。
