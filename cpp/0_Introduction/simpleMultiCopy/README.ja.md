# simpleMultiCopy - Simple Multi Copy and Compute - Japanese Learning Guide
...
Original README headings: `simpleMultiCopy - Simple Multi Copy and Compute`, `Description`, `Key Concepts`, `Supported SM Architectures`, `Supported OSes`, `Supported CPU Architecture`, `CUDA APIs involved`, `[CUDA Runtime API](http://docs.nvidia.com/cuda/cuda-runtime-api/index.html)`, `Prerequisites`, `References (for more details)`

> **日本語**
> `cpp/0_Introduction/simpleMultiCopy` は `C++/CUDA` の CUDA sample です。英語 README を正本として残し、この guide では実装を読む順番と CUDA の学習ポイントを日本語で補足します。
>
> **学習メモ**
...
English anchor: read `simpleMultiCopy` as a focused example of the CUDA concepts used in `cpp/0_Introduction/simpleMultiCopy`.
> **日本語**
> この sample の目的は、`simpleMultiCopy` の小さな実装を通して Streams And Events, Performance, Memory, Kernel Launch And Indexing, Execution Model を具体的に追うことです。
>
> **学習メモ**
> 最初に `simpleMultiCopy.cu` を読み、setup、GPU work、sync、validation、cleanup の境界を source 内で対応させます。
...
- `simpleMultiCopy.cu`: CUDA source containing kernels, Runtime API calls, or device-side helper code.
...
- Open `simpleMultiCopy.cu` first and locate the host-side setup or Python entry point.
...
```bash
cmake -S . -B build
cmake --build build --target simpleMultiCopy
ctest --test-dir build -R simpleMultiCopy
```
