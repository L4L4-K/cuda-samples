# simpleP2P - Simple Peer-to-Peer Transfers with Multi-GPU - Japanese Learning Guide
...
Original README headings: `simpleP2P - Simple Peer-to-Peer Transfers with Multi-GPU`, `Description`, `Key Concepts`, `Supported SM Architectures`, `Supported OSes`, `Supported CPU Architecture`, `CUDA APIs involved`, `[CUDA Runtime API](http://docs.nvidia.com/cuda/cuda-runtime-api/index.html)`, `Dependencies needed to build/run`, `Prerequisites`, `References (for more details)`

> **日本語**
> `cpp/0_Introduction/simpleP2P` は `C++/CUDA` の CUDA sample です。英語 README を正本として残し、この guide では実装を読む順番と CUDA の学習ポイントを日本語で補足します。
>
> **学習メモ**
...
English anchor: read `simpleP2P` as a focused example of the CUDA concepts used in `cpp/0_Introduction/simpleP2P`.
> **日本語**
> この sample の目的は、`simpleP2P` の小さな実装を通して Multi-GPU, P2P, And IPC, Streams And Events, Unified Memory, Performance, Memory を具体的に追うことです。
>
> **学習メモ**
> 最初に `simpleP2P.cu` を読み、setup、GPU work、sync、validation、cleanup の境界を source 内で対応させます。
...
- `simpleP2P.cu`: CUDA source containing kernels, Runtime API calls, or device-side helper code.
...
- Open `simpleP2P.cu` first and locate the host-side setup or Python entry point.
...
- `simpleP2P.cu`: focus on `cudaSetDevice`, `cudaMemcpy`, `CUDA`, `cudaMemcpyDefault`, `launch`.
...
```bash
cmake -S . -B build
cmake --build build --target simpleP2P
ctest --test-dir build -R simpleP2P
```
