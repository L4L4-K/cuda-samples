# clock_nvrtc - Clock libNVRTC - Japanese Learning Guide
...
Original README headings: `clock_nvrtc - Clock libNVRTC`, `Description`, `Key Concepts`, `Supported SM Architectures`, `Supported OSes`, `Supported CPU Architecture`, `CUDA APIs involved`, `[CUDA Driver API](http://docs.nvidia.com/cuda/cuda-driver-api/index.html)`, `[CUDA Runtime API](http://docs.nvidia.com/cuda/cuda-runtime-api/index.html)`, `Dependencies needed to build/run`, `Prerequisites`, `References (for more details)`

> **日本語**
> `cpp/0_Introduction/clock_nvrtc` は `C++/CUDA` の CUDA sample です。英語 README を正本として残し、この guide では実装を読む順番と CUDA の学習ポイントを日本語で補足します。
>
> **学習メモ**
...
English anchor: read `clock_nvrtc` as a focused example of the CUDA concepts used in `cpp/0_Introduction/clock_nvrtc`.
> **日本語**
> この sample の目的は、`clock_nvrtc` の小さな実装を通して Runtime, Driver, And NVRTC, Multi-GPU, P2P, And IPC, Shared Memory, Synchronization And Atomics, Performance を具体的に追うことです。
>
> **学習メモ**
> 最初に `clock.cpp, clock_kernel.cu` を読み、setup、GPU work、sync、validation、cleanup の境界を source 内で対応させます。
...
- `clock.cpp`: Host-side setup, API calls, validation, and cleanup.
- `clock_kernel.cu`: CUDA source containing kernels, Runtime API calls, or device-side helper code.
...
- Open `clock.cpp` first and locate the host-side setup or Python entry point.
...
```bash
cmake -S . -B build
cmake --build build --target clock_nvrtc
ctest --test-dir build -R clock_nvrtc
```
