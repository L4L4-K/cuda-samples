# systemWideAtomics - System wide Atomics - Japanese Learning Guide
...
Original README headings: `systemWideAtomics - System wide Atomics`, `Description`, `Key Concepts`, `Supported SM Architectures`, `Supported OSes`, `Supported CPU Architecture`, `CUDA APIs involved`, `[CUDA Runtime API](http://docs.nvidia.com/cuda/cuda-runtime-api/index.html)`, `Dependencies needed to build/run`, `Prerequisites`, `References (for more details)`

> **日本語**
> `cpp/0_Introduction/systemWideAtomics` は `C++/CUDA` の CUDA sample です。英語 README を正本として残し、この guide では実装を読む順番と CUDA の学習ポイントを日本語で補足します。
>
> **学習メモ**
...
English anchor: read `systemWideAtomics` as a focused example of the CUDA concepts used in `cpp/0_Introduction/systemWideAtomics`.
> **日本語**
> この sample の目的は、`systemWideAtomics` の小さな実装を通して Unified Memory, Synchronization And Atomics, Memory, Kernel Launch And Indexing, Execution Model を具体的に追うことです。
>
> **学習メモ**
> 最初に `systemWideAtomics.cu` を読み、setup、GPU work、sync、validation、cleanup の境界を source 内で対応させます。
...
- `systemWideAtomics.cu`: CUDA source containing kernels, Runtime API calls, or device-side helper code.
...
- Open `systemWideAtomics.cu` first and locate the host-side setup or Python entry point.
...
- `systemWideAtomics.cu`: focus on `cudaDeviceSynchronize`, `launch`, `atomicKernel`, `blockDim`, `blockIdx`.
...
| `atomicKernel` | 複数 thread が同じ address を更新する箇所です。競合と順序の意味を確認します。 |
| `blockDim` | thread/block index から担当 data を決める記号です。境界チェックと一緒に読みます。 |
| `blockIdx` | thread/block index から担当 data を決める記号です。境界チェックと一緒に読みます。 |
| `threadIdx` | thread/block index から担当 data を決める記号です。境界チェックと一緒に読みます。 |
| `atomicKernel_CPU` | 複数 thread が同じ address を更新する箇所です。競合と順序の意味を確認します。 |
| `cudaDeviceGetAttribute` | この sample の中心 API/概念です。入力、所有権、同期、検証との関係を確認します。 |
| `atomics` | 複数 thread が同じ address を更新する箇所です。競合と順序の意味を確認します。 |
| `atomicAdd_system` | 複数 thread が同じ address を更新する箇所です。競合と順序の意味を確認します。 |
...
```bash
cmake -S . -B build
cmake --build build --target systemWideAtomics
ctest --test-dir build -R systemWideAtomics
```
