# graphConditionalNodes - Graph Conditional Nodes - Japanese Learning Guide
...
Original README headings: `graphConditionalNodes - Graph Conditional Nodes`, `Description`, `Key Concepts`, `Supported SM Architectures`, `Supported OSes`, `Supported CPU Architecture`, `CUDA APIs involved`, `[CUDA Runtime API](http://docs.nvidia.com/cuda/cuda-runtime-api/index.html)`, `Prerequisites`, `References (for more details)`

> **日本語**
> `cpp/3_CUDA_Features/graphConditionalNodes` は `C++/CUDA` の CUDA sample です。英語 README を正本として残し、この guide では実装を読む順番と CUDA の学習ポイントを日本語で補足します。
>
> **学習メモ**
...
English anchor: read `graphConditionalNodes` as a focused example of the CUDA concepts used in `cpp/3_CUDA_Features/graphConditionalNodes`.
> **日本語**
> この sample の目的は、`graphConditionalNodes` の小さな実装を通して Runtime, Driver, And NVRTC, CUDA Graphs, Streams And Events, Synchronization And Atomics, Memory を具体的に追うことです。
>
> **学習メモ**
> 最初に `graphConditionalNodes.cu` を読み、setup、GPU work、sync、validation、cleanup の境界を source 内で対応させます。
...
- `graphConditionalNodes.cu`: CUDA source containing kernels, Runtime API calls, or device-side helper code.
...
- Open `graphConditionalNodes.cu` first and locate the host-side setup or Python entry point.
...
| `cudaGraphConditionalHandle` | CUDA Graph の node、capture、instantiate、launch、update の境界を表します。 |
...
| `cudaGraphNodeParams` | CUDA Graph の node、capture、instantiate、launch、update の境界を表します。 |
| `cudaFree` | resource lifetime を閉じる API です。未完了 work が残っていないかを確認します。 |
| `cudaGraphConditionalHandleCreate` | CUDA Graph の node、capture、instantiate、launch、update の境界を表します。 |
...
```bash
cmake -S . -B build
cmake --build build --target graphConditionalNodes
ctest --test-dir build -R graphConditionalNodes
```
