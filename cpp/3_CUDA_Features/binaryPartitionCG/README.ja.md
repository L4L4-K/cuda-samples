# binaryPartitionCG - Binary Partition Cooperative Groups - Japanese Learning Guide
...
Original README headings: `binaryPartitionCG - Binary Partition Cooperative Groups`, `Description`, `Key Concepts`, `Supported SM Architectures`, `Supported OSes`, `Supported CPU Architecture`, `CUDA APIs involved`, `[CUDA Runtime API](http://docs.nvidia.com/cuda/cuda-runtime-api/index.html)`, `Prerequisites`, `References (for more details)`

> **日本語**
> `cpp/3_CUDA_Features/binaryPartitionCG` は `C++/CUDA` の CUDA sample です。英語 README を正本として残し、この guide では実装を読む順番と CUDA の学習ポイントを日本語で補足します。
>
> **学習メモ**
...
English anchor: read `binaryPartitionCG` as a focused example of the CUDA concepts used in `cpp/3_CUDA_Features/binaryPartitionCG`.
> **日本語**
> この sample の目的は、`binaryPartitionCG` の小さな実装を通して Cooperative Groups, Streams And Events, Synchronization And Atomics, Performance, Memory を具体的に追うことです。
>
> **学習メモ**
> 最初に `binaryPartitionCG.cu` を読み、setup、GPU work、sync、validation、cleanup の境界を source 内で対応させます。
...
- `binaryPartitionCG.cu`: CUDA source containing kernels, Runtime API calls, or device-side helper code.
...
- Open `binaryPartitionCG.cu` first and locate the host-side setup or Python entry point.
...
- `binaryPartitionCG.cu`: focus on `cudaMallocHost`, `cudaMalloc`, `cudaMemcpyAsync`, `cudaFreeHost`, `atomicAdd`.
...
```bash
cmake -S . -B build
cmake --build build --target binaryPartitionCG
ctest --test-dir build -R binaryPartitionCG
```
