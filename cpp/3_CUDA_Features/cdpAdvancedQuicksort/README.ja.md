# cdpAdvancedQuicksort - Advanced Quicksort (CUDA Dynamic Parallelism) - Japanese Learning Guide
...
Original README headings: `cdpAdvancedQuicksort - Advanced Quicksort (CUDA Dynamic Parallelism)`, `Description`, `Key Concepts`, `Supported SM Architectures`, `Supported OSes`, `Supported CPU Architecture`, `CUDA APIs involved`, `[CUDA Runtime API](http://docs.nvidia.com/cuda/cuda-runtime-api/index.html)`, `Dependencies needed to build/run`, `Prerequisites`, `References (for more details)`

> **日本語**
> `cpp/3_CUDA_Features/cdpAdvancedQuicksort` は `C++/CUDA` の CUDA sample です。英語 README を正本として残し、この guide では実装を読む順番と CUDA の学習ポイントを日本語で補足します。
>
> **学習メモ**
...
English anchor: read `cdpAdvancedQuicksort` as a focused example of the CUDA concepts used in `cpp/3_CUDA_Features/cdpAdvancedQuicksort`.
> **日本語**
> この sample の目的は、`cdpAdvancedQuicksort` の小さな実装を通して Cooperative Groups, Shared Memory, Streams And Events, Synchronization And Atomics, Performance を具体的に追うことです。
>
> **学習メモ**
> 最初に `cdpAdvancedQuicksort.cu, cdpBitonicSort.cu, cdpQuicksort.h` を読み、setup、GPU work、sync、validation、cleanup の境界を source 内で対応させます。
...
- `cdpAdvancedQuicksort.cu`: CUDA source containing kernels, Runtime API calls, or device-side helper code.
- `cdpBitonicSort.cu`: CUDA source containing kernels, Runtime API calls, or device-side helper code.
- `cdpQuicksort.h`: Host/device declarations, helper types, constants, or library wrappers.
...
- Open `cdpAdvancedQuicksort.cu` first and locate the host-side setup or Python entry point.
...
- `cdpAdvancedQuicksort.cu`: focus on `launch`, `atomicData`, `atomicDataStack`, `atomicAdd`, `atomic`.
- `cdpBitonicSort.cu`: focus on `threadIdx`, `blockDim`, `__shared__`, `launch`.
- `cdpQuicksort.h`: focus on `atomic`, `launch`, `atomicData`.
...
Source: cpp/3_CUDA_Features/cdpAdvancedQuicksort/CMakeLists.txt:1-47
```cmake
...
> JP: この抜粋は `cpp/3_CUDA_Features/cdpAdvancedQuicksort/CMakeLists.txt` の実コードです。setup、allocation、transfer、GPU work、sync、validation、cleanup のどの境界を示すかを、行番号と一緒に確認します。

### `cdpAdvancedQuicksort.cu`

Source: cpp/3_CUDA_Features/cdpAdvancedQuicksort/cdpAdvancedQuicksort.cu:44-62
```cuda
...
> JP: この抜粋は `cpp/3_CUDA_Features/cdpAdvancedQuicksort/cdpAdvancedQuicksort.cu` の実コードです。setup、allocation、transfer、GPU work、sync、validation、cleanup のどの境界を示すかを、行番号と一緒に確認します。

Source: cpp/3_CUDA_Features/cdpAdvancedQuicksort/cdpAdvancedQuicksort.cu:108-127
```cuda
{
...
Source: cpp/3_CUDA_Features/cdpAdvancedQuicksort/cdpAdvancedQuicksort.cu:149-168
```cuda
{
...
Source: cpp/3_CUDA_Features/cdpAdvancedQuicksort/cdpAdvancedQuicksort.cu:243-262
```cuda
...
Source: cpp/3_CUDA_Features/cdpAdvancedQuicksort/cdpBitonicSort.cu:37-55
```cuda
#include <cooperative_groups.h>
#include <stdio.h>
...
> JP: この抜粋は `cpp/3_CUDA_Features/cdpAdvancedQuicksort/cdpBitonicSort.cu` の実コードです。setup、allocation、transfer、GPU work、sync、validation、cleanup のどの境界を示すかを、行番号と一緒に確認します。

Source: cpp/3_CUDA_Features/cdpAdvancedQuicksort/cdpBitonicSort.cu:88-129
```cuda
//
////////////////////////////////////////////////////////////////////////////////
...
Source: cpp/3_CUDA_Features/cdpAdvancedQuicksort/cdpQuicksort.h:31-78
```cpp
#ifndef QUICKSORT_H
#define QUICKSORT_H
...
> JP: この抜粋は `cpp/3_CUDA_Features/cdpAdvancedQuicksort/cdpQuicksort.h` の実コードです。setup、allocation、transfer、GPU work、sync、validation、cleanup のどの境界を示すかを、行番号と一緒に確認します。
...
| `atomicData` | 複数 thread が同じ address を更新する箇所です。競合と順序の意味を確認します。 |
| `atomic` | 複数 thread が同じ address を更新する箇所です。競合と順序の意味を確認します。 |
| `cudaMalloc` | device 側 storage を確保する API です。対応する cleanup と byte size を確認します。 |
| `cudaFree` | resource lifetime を閉じる API です。未完了 work が残っていないかを確認します。 |
| `atomicDataStack` | 複数 thread が同じ address を更新する箇所です。競合と順序の意味を確認します。 |
...
| `cudaPeekAtLastError` | この sample の中心 API/概念です。入力、所有権、同期、検証との関係を確認します。 |
...
```bash
cmake -S . -B build
cmake --build build --target cdpAdvancedQuicksort
ctest --test-dir build -R cdpAdvancedQuicksort
```
