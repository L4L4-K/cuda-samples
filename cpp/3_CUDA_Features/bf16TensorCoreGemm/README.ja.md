# bf16TensorCoreGemm - bfloat16 Tensor Core GEMM - Japanese Learning Guide
...
Original README headings: `bf16TensorCoreGemm - bfloat16 Tensor Core GEMM`, `Description`, `Key Concepts`, `Supported SM Architectures`, `Supported OSes`, `Supported CPU Architecture`, `CUDA APIs involved`, `[CUDA Runtime API](http://docs.nvidia.com/cuda/cuda-runtime-api/index.html)`, `Dependencies needed to build/run`, `Prerequisites`, `References (for more details)`

> **日本語**
> `cpp/3_CUDA_Features/bf16TensorCoreGemm` は `C++/CUDA` の CUDA sample です。英語 README を正本として残し、この guide では実装を読む順番と CUDA の学習ポイントを日本語で補足します。
>
> **学習メモ**
...
English anchor: read `bf16TensorCoreGemm` as a focused example of the CUDA concepts used in `cpp/3_CUDA_Features/bf16TensorCoreGemm`.
> **日本語**
> この sample の目的は、`bf16TensorCoreGemm` の小さな実装を通して Tensor Cores And WMMA, Shared Memory, Streams And Events, Synchronization And Atomics, Performance を具体的に追うことです。
>
> **学習メモ**
> 最初に `bf16TensorCoreGemm.cu` を読み、setup、GPU work、sync、validation、cleanup の境界を source 内で対応させます。
...
- `bf16TensorCoreGemm.cu`: CUDA source containing kernels, Runtime API calls, or device-side helper code.
...
- Open `bf16TensorCoreGemm.cu` first and locate the host-side setup or Python entry point.
...
- `bf16TensorCoreGemm.cu`: focus on `__syncthreads`, `blockDim`, `threadIdx`, `gridDim`, `cudaMemcpy`.
...
Source: cpp/3_CUDA_Features/bf16TensorCoreGemm/bf16TensorCoreGemm.cu:56-79
```cuda
...
> JP: この抜粋は `cpp/3_CUDA_Features/bf16TensorCoreGemm/bf16TensorCoreGemm.cu` の実コードです。setup、allocation、transfer、GPU work、sync、validation、cleanup のどの境界を示すかを、行番号と一緒に確認します。

Source: cpp/3_CUDA_Features/bf16TensorCoreGemm/bf16TensorCoreGemm.cu:201-220
```cuda
...
Source: cpp/3_CUDA_Features/bf16TensorCoreGemm/bf16TensorCoreGemm.cu:692-711
```cuda
...
Source: cpp/3_CUDA_Features/bf16TensorCoreGemm/bf16TensorCoreGemm.cu:719-738
```cuda
#if CPU_DEBUG
...
| `cudaFuncSetAttribute` | この sample の中心 API/概念です。入力、所有権、同期、検証との関係を確認します。 |
...
```bash
cmake -S . -B build
cmake --build build --target bf16TensorCoreGemm
ctest --test-dir build -R bf16TensorCoreGemm
```
...
- `__syncthreads` の直前と直後で、どの memory/resource が有効になったかをメモする。
