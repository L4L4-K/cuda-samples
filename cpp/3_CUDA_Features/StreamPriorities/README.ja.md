# StreamPriorities - Stream Priorities - Japanese Learning Guide
...
English anchor: read `StreamPriorities` as a focused example of the CUDA concepts used in `cpp/3_CUDA_Features/StreamPriorities`.
> **日本語**
> この sample の目的は、`StreamPriorities` の小さな実装を通して Streams And Events, Performance, Memory, Kernel Launch And Indexing, Execution Model を具体的に追うことです。
>
> **学習メモ**
> 最初に `StreamPriorities.cu` を読み、setup、GPU work、sync、validation、cleanup の境界を source 内で対応させます。
...
- `StreamPriorities.cu`: CUDA source containing kernels, Runtime API calls, or device-side helper code.
...
- `StreamPriorities.cu`: focus on `cudaMalloc`, `cudaMemcpy`, `cudaEvent_t`, `cudaEventCreate`, `cudaEventRecord`.
...
| `cudaStreamCreateWithPriority` | 非同期 work の順序、overlap、計測範囲を表す API です。 |
...
| `cudaDeviceGetStreamPriorityRange` | 非同期 work の順序、overlap、計測範囲を表す API です。 |
...
```bash
cmake -S . -B build
cmake --build build --target StreamPriorities
ctest --test-dir build -R StreamPriorities
```
