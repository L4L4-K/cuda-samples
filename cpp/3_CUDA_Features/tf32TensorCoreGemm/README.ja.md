# tf32TensorCoreGemm - tf32 Tensor Core GEMM - Japanese Learning Guide
...
Original README headings: `tf32TensorCoreGemm - tf32 Tensor Core GEMM`, `Description`, `Key Concepts`, `Supported SM Architectures`, `Supported OSes`, `Supported CPU Architecture`, `CUDA APIs involved`, `[CUDA Runtime API](http://docs.nvidia.com/cuda/cuda-runtime-api/index.html)`, `Dependencies needed to build/run`, `Prerequisites`, `References (for more details)`

> **日本語**
> `cpp/3_CUDA_Features/tf32TensorCoreGemm` は `C++/CUDA` の CUDA sample です。英語 README を正本として残し、この guide では実装を読む順番と CUDA の学習ポイントを日本語で補足します。
>
> **学習メモ**
...
English anchor: read `tf32TensorCoreGemm` as a focused example of the CUDA concepts used in `cpp/3_CUDA_Features/tf32TensorCoreGemm`.
> **日本語**
> この sample の目的は、`tf32TensorCoreGemm` の小さな実装を通して Tensor Cores And WMMA, Shared Memory, Streams And Events, Synchronization And Atomics, Performance を具体的に追うことです。
>
> **学習メモ**
> 最初に `tf32TensorCoreGemm.cu` を読み、setup、GPU work、sync、validation、cleanup の境界を source 内で対応させます。
...
- `tf32TensorCoreGemm.cu`: CUDA source containing kernels, Runtime API calls, or device-side helper code.
...
- Open `tf32TensorCoreGemm.cu` first and locate the host-side setup or Python entry point.
...
- `tf32TensorCoreGemm.cu`: focus on `__syncthreads`, `blockDim`, `threadIdx`, `gridDim`, `cudaMemcpy`.
...
```bash
cmake -S . -B build
cmake --build build --target tf32TensorCoreGemm
ctest --test-dir build -R tf32TensorCoreGemm
```
