# memMapIPCDrv - Memmap IPC Driver API - Japanese Learning Guide
...
Original README headings: `memMapIPCDrv - Memmap IPC Driver API`, `Description`, `Key Concepts`, `Supported SM Architectures`, `Supported OSes`, `Supported CPU Architecture`, `CUDA APIs involved`, `[CUDA Driver API](http://docs.nvidia.com/cuda/cuda-driver-api/index.html)`, `Dependencies needed to build/run`, `Prerequisites`, `References (for more details)`

> **日本語**
> `cpp/3_CUDA_Features/memMapIPCDrv` は `C++/CUDA` の CUDA sample です。英語 README を正本として残し、この guide では実装を読む順番と CUDA の学習ポイントを日本語で補足します。
>
> **学習メモ**
...
English anchor: read `memMapIPCDrv` as a focused example of the CUDA concepts used in `cpp/3_CUDA_Features/memMapIPCDrv`.
> **日本語**
> この sample の目的は、`memMapIPCDrv` の小さな実装を通して Runtime, Driver, And NVRTC, Multi-GPU, P2P, And IPC, Streams And Events, Synchronization And Atomics, Performance を具体的に追うことです。
>
> **学習メモ**
> 最初に `memMapIpc.cpp, memMapIpc_kernel.cu` を読み、setup、GPU work、sync、validation、cleanup の境界を source 内で対応させます。
...
- `memMapIpc.cpp`: Host-side setup, API calls, validation, and cleanup.
- `memMapIpc_kernel.cu`: CUDA source containing kernels, Runtime API calls, or device-side helper code.
...
- Open `memMapIpc.cpp` first and locate the host-side setup or Python entry point.
...
- `memMapIpc.cpp`: focus on `cuModule`, `cuDeviceGetAttribute`, `CUDA`, `cuMemCreate`, `CUdeviceptr`.
- `memMapIpc_kernel.cu`: focus on `blockDim`, `blockIdx`, `threadIdx`, `launch`, `Device`.
...
| `cuDeviceGet` | Driver API の handle 境界です。context/module/function と error code を追います。 |
...
| `cuStreamSynchronize` | 非同期 work の順序、overlap、計測範囲を表す API です。 |
| `cuMemcpyDtoHAsync` | host/device 間の転送、初期化、または visibility を作る API です。方向と Async の順序を確認します。 |
...
```bash
cmake -S . -B build
cmake --build build --target memMapIPCDrv
ctest --test-dir build -R memMapIPCDrv
```
...
- `cuMemMap` の直前と直後で、どの memory/resource が有効になったかをメモする。
