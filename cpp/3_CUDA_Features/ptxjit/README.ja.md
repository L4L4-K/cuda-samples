# ptxjit - PTX Just-in-Time compilation - Japanese Learning Guide
...
Original README headings: `ptxjit - PTX Just-in-Time compilation`, `Description`, `Key Concepts`, `Supported SM Architectures`, `Supported OSes`, `Supported CPU Architecture`, `CUDA APIs involved`, `[CUDA Driver API](http://docs.nvidia.com/cuda/cuda-driver-api/index.html)`, `[CUDA Runtime API](http://docs.nvidia.com/cuda/cuda-runtime-api/index.html)`, `Prerequisites`, `References (for more details)`

> **日本語**
> `cpp/3_CUDA_Features/ptxjit` は `C++/CUDA` の CUDA sample です。英語 README を正本として残し、この guide では実装を読む順番と CUDA の学習ポイントを日本語で補足します。
>
> **学習メモ**
...
English anchor: read `ptxjit` as a focused example of the CUDA concepts used in `cpp/3_CUDA_Features/ptxjit`.
> **日本語**
> この sample の目的は、`ptxjit` の小さな実装を通して Runtime, Driver, And NVRTC, Multi-GPU, P2P, And IPC, Streams And Events, Synchronization And Atomics, Memory を具体的に追うことです。
>
> **学習メモ**
> 最初に `ptxjit.cpp, ptxjit_kernel.cu` を読み、setup、GPU work、sync、validation、cleanup の境界を source 内で対応させます。
...
- `ptxjit.cpp`: Host-side setup, API calls, validation, and cleanup.
- `ptxjit_kernel.cu`: CUDA source containing kernels, Runtime API calls, or device-side helper code.
...
- Open `ptxjit.cpp` first and locate the host-side setup or Python entry point.
...
- `ptxjit.cpp`: focus on `CUDA`, `cuOut`, `CUDART_VERSION`, `CUmodule`, `CUfunction`.
- `ptxjit_kernel.cu`: focus on `blockIdx`, `blockDim`, `threadIdx`, `launch`.
...
| `cuOut` | Driver API の handle 境界です。context/module/function と error code を追います。 |
| `cuLinkCreate` | Driver API の handle 境界です。context/module/function と error code を追います。 |
| `cuLinkAddData` | Driver API の handle 境界です。context/module/function と error code を追います。 |
| `cuLinkComplete` | Driver API の handle 境界です。context/module/function と error code を追います。 |
...
| `cuLinkDestroy` | resource lifetime を閉じる API です。未完了 work が残っていないかを確認します。 |
...
| `cuModuleUnload` | Driver API の handle 境界です。context/module/function と error code を追います。 |
...
```bash
cmake -S . -B build
cmake --build build --target ptxjit
ctest --test-dir build -R ptxjit
```
