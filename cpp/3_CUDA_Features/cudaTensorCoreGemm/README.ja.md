# cudaTensorCoreGemm - CUDA Tensor Core GEMM - Japanese Learning Guide
...
Original README headings: `cudaTensorCoreGemm - CUDA Tensor Core GEMM`, `Description`, `Key Concepts`, `Supported SM Architectures`, `Supported OSes`, `Supported CPU Architecture`, `CUDA APIs involved`, `[CUDA Runtime API](http://docs.nvidia.com/cuda/cuda-runtime-api/index.html)`, `Prerequisites`, `References (for more details)`

> **日本語**
> `cpp/3_CUDA_Features/cudaTensorCoreGemm` は `C++/CUDA` の CUDA sample です。英語 README を正本として残し、この guide では実装を読む順番と CUDA の学習ポイントを日本語で補足します。
>
> **学習メモ**
...
English anchor: read `cudaTensorCoreGemm` as a focused example of the CUDA concepts used in `cpp/3_CUDA_Features/cudaTensorCoreGemm`.
> **日本語**
> この sample の目的は、`cudaTensorCoreGemm` の小さな実装を通して Tensor Cores And WMMA, Shared Memory, Streams And Events, Synchronization And Atomics, Performance を具体的に追うことです。
>
> **学習メモ**
> 最初に `cudaTensorCoreGemm.cu` を読み、setup、GPU work、sync、validation、cleanup の境界を source 内で対応させます。
...
- `cudaTensorCoreGemm.cu`: CUDA source containing kernels, Runtime API calls, or device-side helper code.
...
- Open `cudaTensorCoreGemm.cu` first and locate the host-side setup or Python entry point.
...
- `cudaTensorCoreGemm.cu`: focus on `blockDim`, `__syncthreads`, `cudaMemcpy`, `threadIdx`, `gridDim`.
...
Source: cpp/3_CUDA_Features/cudaTensorCoreGemm/cudaTensorCoreGemm.cu:59-82
```cuda
...
> JP: この抜粋は `cpp/3_CUDA_Features/cudaTensorCoreGemm/cudaTensorCoreGemm.cu` の実コードです。setup、allocation、transfer、GPU work、sync、validation、cleanup のどの境界を示すかを、行番号と一緒に確認します。

Source: cpp/3_CUDA_Features/cudaTensorCoreGemm/cudaTensorCoreGemm.cu:195-214
```cuda
    }
}
...
Source: cpp/3_CUDA_Features/cudaTensorCoreGemm/cudaTensorCoreGemm.cu:476-495
```cuda
...
Source: cpp/3_CUDA_Features/cudaTensorCoreGemm/cudaTensorCoreGemm.cu:504-523
```cuda
#if CPU_DEBUG
...
```bash
cmake -S . -B build
cmake --build build --target cudaTensorCoreGemm
ctest --test-dir build -R cudaTensorCoreGemm
```
