...
- `graphMemoryFootprint.cu`: focus on `CUDA`, `cudaGraphExec_t`, `cudaGraphLaunch`, `cudaStream_t`, `cudaStreamSynchronize`.
...
- `cudaGraphLaunch` の直前と直後で、どの memory/resource が有効になったかをメモする。
