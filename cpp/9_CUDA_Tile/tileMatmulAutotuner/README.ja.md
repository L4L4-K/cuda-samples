...
A CUDA Tile C++ sample demonstrating how to autotune tile sizes and optimization hints for matrix multiplication with FP16 inputs and FP32 accumulation when compiling with nvrtc or nvcc. The sample explores combinations of tile sizes and optimization hints, compiles the Tile C++ matrix multiplication kernel, executes it, and reports the best measured configuration. The launch grid is derived from the selected tile size and matrix dimensions.

Original README headings: `tileMatmulAutotuner`, `Description`, `Running`, `Run autotuner. Validation is disabled by default.`, `Select a backend`, `Enable CPU validation`, `To run faster, skip warmups and set iteration to 1`, `Show all options`

> **日本語**
> `cpp/9_CUDA_Tile/tileMatmulAutotuner` は `9_CUDA_Tile` に属する CUDA sample です。英語 README の目的と手順を残し、ここでは日本語で実装の読み方を補足します。
>
> **学習メモ**
...
English anchor: this sample demonstrates `tileMatmulAutotuner` and should be read together with the original README and source files.
...
- Tensor Core/WMMA/MMA の data type、tile、layout 条件を確認します。
...
- [Tensor Cores And WMMA](../../../docs_ja/themes/tensor_cores_wmma.md): Tensor Core/WMMA/MMA の data type、tile、layout 条件を確認します。
