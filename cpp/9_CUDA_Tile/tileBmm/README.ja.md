# tileBmm - Japanese Learning Guide
...
This sample demonstrates a static-persistent batched matrix multiplication (BMM) using CUDA Tile C++. Given inputs A of shape (Q, M, K) and B of shape (Q, K, N), the kernel computes C = A x B of shape (Q, M, N). The grid launches a fixed number of persistent blocks sized from the device's SM count, and each block walks the (M, N, Q-chunk) tile space via a grid-stride loop. The batch dimension is tiled by BLOCK_SIZE_Q so every iteration issues

a single rank-3 batched cuda::tiles::mma per K-step over tiles of shape (BLOCK_SIZE_Q, BLOCK_SIZE_M, BLOCK_SIZE_K) and (BLOCK_SIZE_Q, BLOCK_SIZE_K, BLOCK_SIZE_N). Grouped ordering on (pid_m, pid_n) gives L2 reuse. The accumulator is kept in float32 for precision, and masked loads/stores handle tiles that overhang the matrix or batch boundaries. Inputs and outputs use __half precision.

Original README headings: `tileBmm`, `Description`, `Expected Output`, `Prerequisites`

> **日本語**
> `cpp/9_CUDA_Tile/tileBmm` は `C++/CUDA` の CUDA sample です。英語 README を正本として残し、この guide では実装を読む順番と CUDA の学習ポイントを日本語で補足します。
>
> **学習メモ**
...
English anchor: read `tileBmm` as a focused example of the CUDA concepts used in `cpp/9_CUDA_Tile/tileBmm`.
> **日本語**
> この sample の目的は、`tileBmm` の小さな実装を通して Runtime, Driver, And NVRTC, Multi-GPU, P2P, And IPC, Tensor Cores And WMMA, Shared Memory, Streams And Events を具体的に追うことです。
>
> **学習メモ**
> 最初に `tileBmm.cu` を読み、setup、GPU work、sync、validation、cleanup の境界を source 内で対応させます。
...
- `tileBmm.cu`: CUDA source containing kernels, Runtime API calls, or device-side helper code.
...
- Open `tileBmm.cu` first and locate the host-side setup or Python entry point.
...
- `tileBmm.cu`: focus on `launch`, `cudaMalloc`, `cudaMemcpy`, `cudaMemcpyDeviceToHost`, `cudaFree`.
...
```bash
cmake -S . -B build
cmake --build build --target tileBmm
ctest --test-dir build -R tileBmm
```
