...
This CUDA Tile C++ sample demonstrates basic usage of tile kernels. This code launches a tile kernel using the triple chevron syntax and passes data between SIMT and Tile code through global device memory. Error checks are performed using `cudaGetLastError` to catch kernel launch issues and `cudaDeviceSynchronize` to catch kernel execution issues.

Original README headings: `helloTile`, `Description`, `Expected Output`, `Prerequisites`

> **日本語**
> `cpp/9_CUDA_Tile/helloTile` は `9_CUDA_Tile` に属する CUDA sample です。英語 README の目的と手順を残し、ここでは日本語で実装の読み方を補足します。
>
> **学習メモ**
...
English anchor: this sample demonstrates `helloTile` and should be read together with the original README and source files.
