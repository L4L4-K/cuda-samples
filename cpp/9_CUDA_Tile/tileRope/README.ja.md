# tileRope - Japanese Learning Guide
...
Original README headings: `tileRope`, `Description`, `Expected Output`, `Prerequisites`

> **日本語**
> `cpp/9_CUDA_Tile/tileRope` は `C++/CUDA` の CUDA sample です。英語 README を正本として残し、この guide では実装を読む順番と CUDA の学習ポイントを日本語で補足します。
>
> **学習メモ**
...
English anchor: read `tileRope` as a focused example of the CUDA concepts used in `cpp/9_CUDA_Tile/tileRope`.
> **日本語**
> この sample の目的は、`tileRope` の小さな実装を通して Runtime, Driver, And NVRTC, Multi-GPU, P2P, And IPC, Tensor Cores And WMMA, Shared Memory, Streams And Events を具体的に追うことです。
>
> **学習メモ**
> 最初に `tileRope.cu` を読み、setup、GPU work、sync、validation、cleanup の境界を source 内で対応させます。
...
- `tileRope.cu`: CUDA source containing kernels, Runtime API calls, or device-side helper code.
...
- Open `tileRope.cu` first and locate the host-side setup or Python entry point.
...
- `tileRope.cu`: focus on `cudaMemcpy`, `cudaMemcpyDeviceToHost`, `cudaMalloc`, `cudaFree`, `launch`.
...
```bash
cmake -S . -B build
cmake --build build --target tileRope
ctest --test-dir build -R tileRope
```
