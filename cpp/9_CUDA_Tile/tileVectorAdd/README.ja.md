# tileVectorAdd - Japanese Learning Guide
...
Original README headings: `tileVectorAdd`, `Description`, `Expected Output`, `Prerequisites`

> **日本語**
> `cpp/9_CUDA_Tile/tileVectorAdd` は `C++/CUDA` の CUDA sample です。英語 README を正本として残し、この guide では実装を読む順番と CUDA の学習ポイントを日本語で補足します。
>
> **学習メモ**
...
English anchor: read `tileVectorAdd` as a focused example of the CUDA concepts used in `cpp/9_CUDA_Tile/tileVectorAdd`.
> **日本語**
> この sample の目的は、`tileVectorAdd` の小さな実装を通して Runtime, Driver, And NVRTC, Multi-GPU, P2P, And IPC, Tensor Cores And WMMA, Shared Memory, Performance を具体的に追うことです。
>
> **学習メモ**
> 最初に `tileVectorAdd.cu` を読み、setup、GPU work、sync、validation、cleanup の境界を source 内で対応させます。
...
- `tileVectorAdd.cu`: CUDA source containing kernels, Runtime API calls, or device-side helper code.
...
- Open `tileVectorAdd.cu` first and locate the host-side setup or Python entry point.
...
- `tileVectorAdd.cu`: focus on `cudaMalloc`, `cudaFree`, `launch`, `blockIdx`, `blockDim`.
...
```bash
cmake -S . -B build
cmake --build build --target tileVectorAdd
ctest --test-dir build -R tileVectorAdd
```
