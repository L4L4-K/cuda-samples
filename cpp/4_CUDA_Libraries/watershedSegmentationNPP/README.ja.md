# watershedSegmentationNPP - Watershed Segmentation NPP - Japanese Learning Guide
...
Original README headings: `watershedSegmentationNPP - Watershed Segmentation NPP`, `Description`, `Key Concepts`, `Supported SM Architectures`, `Supported OSes`, `Supported CPU Architecture`, `CUDA APIs involved`, `[CUDA Runtime API](http://docs.nvidia.com/cuda/cuda-runtime-api/index.html)`, `Dependencies needed to build/run`, `Prerequisites`, `References (for more details)`

> **日本語**
> `cpp/4_CUDA_Libraries/watershedSegmentationNPP` は `C++/CUDA` の CUDA sample です。英語 README を正本として残し、この guide では実装を読む順番と CUDA の学習ポイントを日本語で補足します。
>
> **学習メモ**
...
English anchor: read `watershedSegmentationNPP` as a focused example of the CUDA concepts used in `cpp/4_CUDA_Libraries/watershedSegmentationNPP`.
> **日本語**
> この sample の目的は、`watershedSegmentationNPP` の小さな実装を通して Runtime, Driver, And NVRTC, CUDA Libraries, Streams And Events, Performance, Memory を具体的に追うことです。
>
> **学習メモ**
> 最初に `watershedSegmentationNPP.cpp` を読み、setup、GPU work、sync、validation、cleanup の境界を source 内で対応させます。
...
- `watershedSegmentationNPP.cpp`: Host-side setup, API calls, validation, and cleanup.
...
- Open `watershedSegmentationNPP.cpp` first and locate the host-side setup or Python entry point.
...
| `nppStatus` | CUDA library call です。handle/descriptor/workspace と data layout を確認します。 |
| `cudaMemcpy2DAsync` | host/device 間の転送、初期化、または visibility を作る API です。方向と Async の順序を確認します。 |
...
```bash
cmake -S . -B build
cmake --build build --target watershedSegmentationNPP
ctest --test-dir build -R watershedSegmentationNPP
```
...
- `cudaError` の直前と直後で、どの memory/resource が有効になったかをメモする。
