# simpleCUFFT_2d_MGPU - SimpleCUFFT_2d_MGPU - Japanese Learning Guide
...
Original README headings: `simpleCUFFT_2d_MGPU - SimpleCUFFT_2d_MGPU`, `Description`, `Key Concepts`, `Supported SM Architectures`, `Supported OSes`, `Supported CPU Architecture`, `CUDA APIs involved`, `[CUDA Runtime API](http://docs.nvidia.com/cuda/cuda-runtime-api/index.html)`, `Dependencies needed to build/run`, `Prerequisites`, `References (for more details)`

> **日本語**
> `cpp/4_CUDA_Libraries/simpleCUFFT_2d_MGPU` は `C++/CUDA` の CUDA sample です。英語 README を正本として残し、この guide では実装を読む順番と CUDA の学習ポイントを日本語で補足します。
>
> **学習メモ**
...
English anchor: read `simpleCUFFT_2d_MGPU` as a focused example of the CUDA concepts used in `cpp/4_CUDA_Libraries/simpleCUFFT_2d_MGPU`.
> **日本語**
> この sample の目的は、`simpleCUFFT_2d_MGPU` の小さな実装を通して CUDA Libraries, Memory, Kernel Launch And Indexing, Execution Model, Debugging, Profiling, And Testing を具体的に追うことです。
>
> **学習メモ**
> 最初に `simpleCUFFT_2d_MGPU.cu` を読み、setup、GPU work、sync、validation、cleanup の境界を source 内で対応させます。
...
- `simpleCUFFT_2d_MGPU.cu`: CUDA source containing kernels, Runtime API calls, or device-side helper code.
...
- Open `simpleCUFFT_2d_MGPU.cu` first and locate the host-side setup or Python entry point.
...
| - | - |
| `CUFFT_SUCCESS` | Driver API の handle 境界です。context/module/function と error code を追います。 |
| `CUFFT` | Driver API の handle 境界です。context/module/function と error code を追います。 |
| `cudaLibXtDesc` | この sample の中心 API/概念です。入力、所有権、同期、検証との関係を確認します。 |
| `cufftComplex` | Driver API の handle 境界です。context/module/function と error code を追います。 |
| `cufftXtMemcpy` | host/device 間の転送、初期化、または visibility を作る API です。方向と Async の順序を確認します。 |
...
| `cufftXtMalloc` | device 側 storage を確保する API です。対応する cleanup と byte size を確認します。 |
| `cufftXtExecDescriptorC2C` | Driver API の handle 境界です。context/module/function と error code を追います。 |
...
```bash
cmake -S . -B build
cmake --build build --target simpleCUFFT_2d_MGPU
ctest --test-dir build -R simpleCUFFT_2d_MGPU
```
...
- `CUFFT_SUCCESS` の直前と直後で、どの memory/resource が有効になったかをメモする。
