# histEqualizationNPP - Histogram Equalization with NPP - Japanese Learning Guide
...
Original README headings: `histEqualizationNPP - Histogram Equalization with NPP`, `Description`, `Key Concepts`, `Supported SM Architectures`, `Supported OSes`, `Supported CPU Architecture`, `CUDA APIs involved`, `[CUDA Runtime API](http://docs.nvidia.com/cuda/cuda-runtime-api/index.html)`, `Dependencies needed to build/run`, `Prerequisites`, `References (for more details)`

> **日本語**
> `cpp/4_CUDA_Libraries/histEqualizationNPP` は `C++/CUDA` の CUDA sample です。英語 README を正本として残し、この guide では実装を読む順番と CUDA の学習ポイントを日本語で補足します。
>
> **学習メモ**
...
English anchor: read `histEqualizationNPP` as a focused example of the CUDA concepts used in `cpp/4_CUDA_Libraries/histEqualizationNPP`.
> **日本語**
> この sample の目的は、`histEqualizationNPP` の小さな実装を通して Runtime, Driver, And NVRTC, CUDA Libraries, Streams And Events, Performance, Memory を具体的に追うことです。
>
> **学習メモ**
> 最初に `histEqualizationNPP.cpp` を読み、setup、GPU work、sync、validation、cleanup の境界を source 内で対応させます。
...
- `histEqualizationNPP.cpp`: Host-side setup, API calls, validation, and cleanup.
...
- Open `histEqualizationNPP.cpp` first and locate the host-side setup or Python entry point.
...
```bash
cmake -S . -B build
cmake --build build --target histEqualizationNPP
ctest --test-dir build -R histEqualizationNPP
```
