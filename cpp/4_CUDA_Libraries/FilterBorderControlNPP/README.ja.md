# FilterBorderControlNPP - Filter Border Control NPP - Japanese Learning Guide
...
Original README headings: `FilterBorderControlNPP - Filter Border Control NPP`, `Description`, `Key Concepts`, `Supported SM Architectures`, `Supported OSes`, `Supported CPU Architecture`, `CUDA APIs involved`, `[CUDA Runtime API](http://docs.nvidia.com/cuda/cuda-runtime-api/index.html)`, `Dependencies needed to build/run`, `Prerequisites`, `References (for more details)`

> **日本語**
> `cpp/4_CUDA_Libraries/FilterBorderControlNPP` は `C++/CUDA` の CUDA sample です。英語 README を正本として残し、この guide では実装を読む順番と CUDA の学習ポイントを日本語で補足します。
>
> **学習メモ**
...
English anchor: read `FilterBorderControlNPP` as a focused example of the CUDA concepts used in `cpp/4_CUDA_Libraries/FilterBorderControlNPP`.
> **日本語**
> この sample の目的は、`FilterBorderControlNPP` の小さな実装を通して Runtime, Driver, And NVRTC, CUDA Libraries, Streams And Events, Performance, Memory を具体的に追うことです。
>
> **学習メモ**
> 最初に `FilterBorderControlNPP.cpp` を読み、setup、GPU work、sync、validation、cleanup の境界を source 内で対応させます。
...
- `FilterBorderControlNPP.cpp`: Host-side setup, API calls, validation, and cleanup.
...
- `FilterBorderControlNPP.cpp`: focus on `nppStreamCtx`, `CUDA`, `npp`, `nppiFree`, `cudaError`.
...
Source: cpp/4_CUDA_Libraries/FilterBorderControlNPP/CMakeLists.txt:1-23
```cmake
...
> JP: この抜粋は `cpp/4_CUDA_Libraries/FilterBorderControlNPP/CMakeLists.txt` の実コードです。setup、allocation、transfer、GPU work、sync、validation、cleanup のどの境界を示すかを、行番号と一緒に確認します。

Source: cpp/4_CUDA_Libraries/FilterBorderControlNPP/CMakeLists.txt:42-61
```cmake
...
Source: cpp/4_CUDA_Libraries/FilterBorderControlNPP/FilterBorderControlNPP.cpp:37-55
```cpp
...
> JP: この抜粋は `cpp/4_CUDA_Libraries/FilterBorderControlNPP/FilterBorderControlNPP.cpp` の実コードです。setup、allocation、transfer、GPU work、sync、validation、cleanup のどの境界を示すかを、行番号と一緒に確認します。

Source: cpp/4_CUDA_Libraries/FilterBorderControlNPP/FilterBorderControlNPP.cpp:65-102
```cpp
...
Source: cpp/4_CUDA_Libraries/FilterBorderControlNPP/FilterBorderControlNPP.cpp:208-227
```cpp
...
| `npp` | CUDA library call です。handle/descriptor/workspace と data layout を確認します。 |
| `nppiFree` | resource lifetime を閉じる API です。未完了 work が残っていないかを確認します。 |
| `cudaError` | この sample の中心 API/概念です。入力、所有権、同期、検証との関係を確認します。 |
| `cudaDeviceReset` | この sample の中心 API/概念です。入力、所有権、同期、検証との関係を確認します。 |
| `nppiCompareC_16s_C1R_Ctx` | CUDA library call です。handle/descriptor/workspace と data layout を確認します。 |
...
| `nppiGradientVectorPrewittBorder_8u16s_C1R_Ctx` | CUDA library call です。handle/descriptor/workspace と data layout を確認します。 |
| `nppiNormL1` | CUDA library call です。handle/descriptor/workspace と data layout を確認します。 |
| `nppiAbsDiff_8u_C1R_Ctx` | CUDA library call です。handle/descriptor/workspace と data layout を確認します。 |
...
```bash
cmake -S . -B build
cmake --build build --target FilterBorderControlNPP
ctest --test-dir build -R FilterBorderControlNPP
```
...
- `nppStreamCtx` の直前と直後で、どの memory/resource が有効になったかをメモする。
