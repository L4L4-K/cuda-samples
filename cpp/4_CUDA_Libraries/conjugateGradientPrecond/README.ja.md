# conjugateGradientPrecond - Preconditioned Conjugate Gradient - Japanese Learning Guide
...
Original README headings: `conjugateGradientPrecond - Preconditioned Conjugate Gradient`, `Description`, `Key Concepts`, `Supported SM Architectures`, `Supported OSes`, `Supported CPU Architecture`, `CUDA APIs involved`, `[CUDA Runtime API](http://docs.nvidia.com/cuda/cuda-runtime-api/index.html)`, `Dependencies needed to build/run`, `Prerequisites`, `References (for more details)`

> **日本語**
> `cpp/4_CUDA_Libraries/conjugateGradientPrecond` は `C++/CUDA` の CUDA sample です。英語 README を正本として残し、この guide では実装を読む順番と CUDA の学習ポイントを日本語で補足します。
>
> **学習メモ**
...
English anchor: read `conjugateGradientPrecond` as a focused example of the CUDA concepts used in `cpp/4_CUDA_Libraries/conjugateGradientPrecond`.
> **日本語**
> この sample の目的は、`conjugateGradientPrecond` の小さな実装を通して CUDA Libraries, Shared Memory, Memory, Execution Model, Debugging, Profiling, And Testing を具体的に追うことです。
>
> **学習メモ**
...
| `cusparseCreateDnVec` | Driver API の handle 境界です。context/module/function と error code を追います。 |
| `CUSPARSE_INDEX_32I` | Driver API の handle 境界です。context/module/function と error code を追います。 |
| `CUSPARSE_SPSV_ALG_DEFAULT` | Driver API の handle 境界です。context/module/function と error code を追います。 |
...
```bash
cmake -S . -B build
cmake --build build --target conjugateGradientPrecond
ctest --test-dir build -R conjugateGradientPrecond
```
