# batchCUBLAS - batchCUBLAS - Japanese Learning Guide
...
Original README headings: `batchCUBLAS - batchCUBLAS`, `Description`, `Key Concepts`, `Supported SM Architectures`, `Supported OSes`, `Supported CPU Architecture`, `CUDA APIs involved`, `[CUDA Driver API](http://docs.nvidia.com/cuda/cuda-driver-api/index.html)`, `[CUDA Runtime API](http://docs.nvidia.com/cuda/cuda-runtime-api/index.html)`, `Dependencies needed to build/run`, `Prerequisites`, `References (for more details)`

> **日本語**
> `cpp/4_CUDA_Libraries/batchCUBLAS` は `C++/CUDA` の CUDA sample です。英語 README を正本として残し、この guide では実装を読む順番と CUDA の学習ポイントを日本語で補足します。
>
> **学習メモ**
...
English anchor: read `batchCUBLAS` as a focused example of the CUDA concepts used in `cpp/4_CUDA_Libraries/batchCUBLAS`.
> **日本語**
> この sample の目的は、`batchCUBLAS` の小さな実装を通して Runtime, Driver, And NVRTC, CUDA Libraries, Streams And Events, Performance, Memory を具体的に追うことです。
>
> **学習メモ**
> 最初に `batchCUBLAS.cpp, batchCUBLAS.h` を読み、setup、GPU work、sync、validation、cleanup の境界を source 内で対応させます。
...
- `batchCUBLAS.cpp`: Host-side setup, API calls, validation, and cleanup.
- `batchCUBLAS.h`: Host/device declarations, helper types, constants, or library wrappers.
...
- Open `batchCUBLAS.cpp` first and locate the host-side setup or Python entry point.
...
| `CUBLASTEST_FAILED` | Driver API の handle 境界です。context/module/function と error code を追います。 |
| `cublasOperation_t` | Driver API の handle 境界です。context/module/function と error code を追います。 |
| `cudaStatus` | この sample の中心 API/概念です。入力、所有権、同期、検証との関係を確認します。 |
...
| `cuEqual` | Driver API の handle 境界です。context/module/function と error code を追います。 |
| `CUBLASTEST_PASSED` | Driver API の handle 境界です。context/module/function と error code を追います。 |
...
```bash
cmake -S . -B build
cmake --build build --target batchCUBLAS
ctest --test-dir build -R batchCUBLAS
```
