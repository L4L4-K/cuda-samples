# cubDeviceFind - CUB DeviceFind Search Algorithms - Japanese Learning Guide
...
Original README headings: `cubDeviceFind - CUB DeviceFind Search Algorithms`, `Description`, `Key Concepts`, `Supported SM Architectures`, `Supported OSes`, `Supported CPU Architecture`, `CUDA APIs involved`, `[CCCL CUB](https://nvidia.github.io/cccl/unstable/cub/index.html)`, `[CCCL libcu++](https://nvidia.github.io/cccl/unstable/libcudacxx/index.html)`, `[CUDA Runtime API](http://docs.nvidia.com/cuda/cuda-runtime-api/index.html)`, `Dependencies needed to build/run`, `Prerequisites`

> **日本語**
> `cpp/4_CUDA_Libraries/cubDeviceFind` は `C++/CUDA` の CUDA sample です。英語 README を正本として残し、この guide では実装を読む順番と CUDA の学習ポイントを日本語で補足します。
>
> **学習メモ**
...
English anchor: read `cubDeviceFind` as a focused example of the CUDA concepts used in `cpp/4_CUDA_Libraries/cubDeviceFind`.
> **日本語**
> この sample の目的は、`cubDeviceFind` の小さな実装を通して CUDA Libraries, Memory, Execution Model, Debugging, Profiling, And Testing を具体的に追うことです。
>
> **学習メモ**
> 最初に `cubDeviceFind.cu` を読み、setup、GPU work、sync、validation、cleanup の境界を source 内で対応させます。
...
- `cubDeviceFind.cu`: CUDA source containing kernels, Runtime API calls, or device-side helper code.
...
- Open `cubDeviceFind.cu` first and locate the host-side setup or Python entry point.
...
- `cubDeviceFind.cu`: focus on `thrust::device_vector`, `thrust::host_vector`, `cub::DeviceFind::LowerBound`, `cub::DeviceFind::FindIf`, `thrust::raw_pointer_cast`.
...
| `cub::DeviceFind::LowerBound` | Driver API の handle 境界です。context/module/function と error code を追います。 |
| `cub::DeviceFind::FindIf` | Driver API の handle 境界です。context/module/function と error code を追います。 |
...
| `cub::DeviceFind::UpperBound` | Driver API の handle 境界です。context/module/function と error code を追います。 |
...
| `cub::DeviceFind` | Driver API の handle 境界です。context/module/function と error code を追います。 |
...
```bash
cmake -S . -B build
cmake --build build --target cubDeviceFind
ctest --test-dir build -R cubDeviceFind
```
