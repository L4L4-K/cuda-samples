# cubDeviceSegmentedScan - CUB DeviceSegmentedScan - Japanese Learning Guide
...
Original README headings: `cubDeviceSegmentedScan - CUB DeviceSegmentedScan`, `Description`, `Key Concepts`, `Supported SM Architectures`, `Supported OSes`, `Supported CPU Architecture`, `CUDA APIs involved`, `[CCCL CUB](https://nvidia.github.io/cccl/cub/)`, `[CCCL libcu++](https://nvidia.github.io/cccl/libcudacxx/)`, `[CUDA Runtime API](http://docs.nvidia.com/cuda/cuda-runtime-api/index.html)`, `Dependencies needed to build/run`, `Prerequisites`

> **日本語**
> `cpp/4_CUDA_Libraries/cubDeviceSegmentedScan` は `C++/CUDA` の CUDA sample です。英語 README を正本として残し、この guide では実装を読む順番と CUDA の学習ポイントを日本語で補足します。
>
> **学習メモ**
...
English anchor: read `cubDeviceSegmentedScan` as a focused example of the CUDA concepts used in `cpp/4_CUDA_Libraries/cubDeviceSegmentedScan`.
> **日本語**
> この sample の目的は、`cubDeviceSegmentedScan` の小さな実装を通して CUDA Libraries, Streams And Events, Synchronization And Atomics, Memory, Execution Model を具体的に追うことです。
>
> **学習メモ**
> 最初に `cubDeviceSegmentedScan.cu` を読み、setup、GPU work、sync、validation、cleanup の境界を source 内で対応させます。
...
- `cubDeviceSegmentedScan.cu`: CUDA source containing kernels, Runtime API calls, or device-side helper code.
...
- Open `cubDeviceSegmentedScan.cu` first and locate the host-side setup or Python entry point.
...
- `cubDeviceSegmentedScan.cu`: focus on `thrust::device_vector`, `cub::DeviceSegmentedScan::ExclusiveSegmentedSum`, `cudaDeviceSynchronize`, `cub::DeviceSegmentedScan::InclusiveSegmentedScan`, `thrust::raw_pointer_cast`.
...
| `cub::DeviceSegmentedScan` | Driver API の handle 境界です。context/module/function と error code を追います。 |
| `cub::DeviceSegmentedScan::ExclusiveSegmentedSum` | Driver API の handle 境界です。context/module/function と error code を追います。 |
| `cudaDeviceSynchronize` | この sample の中心 API/概念です。入力、所有権、同期、検証との関係を確認します。 |
| `cub::DeviceSegmentedScan::InclusiveSegmentedScan` | Driver API の handle 境界です。context/module/function と error code を追います。 |
...
```bash
cmake -S . -B build
cmake --build build --target cubDeviceSegmentedScan
ctest --test-dir build -R cubDeviceSegmentedScan
```
