# cannyEdgeDetectorNPP - Canny Edge Detector NPP - Japanese Learning Guide
...
Original README headings: `cannyEdgeDetectorNPP - Canny Edge Detector NPP`, `Description`, `Key Concepts`, `Supported SM Architectures`, `Supported OSes`, `Supported CPU Architecture`, `CUDA APIs involved`, `[CUDA Runtime API](http://docs.nvidia.com/cuda/cuda-runtime-api/index.html)`, `Dependencies needed to build/run`, `Prerequisites`, `References (for more details)`

> **日本語**
> `cpp/4_CUDA_Libraries/cannyEdgeDetectorNPP` は `C++/CUDA` の CUDA sample です。英語 README を正本として残し、この guide では実装を読む順番と CUDA の学習ポイントを日本語で補足します。
>
> **学習メモ**
...
English anchor: read `cannyEdgeDetectorNPP` as a focused example of the CUDA concepts used in `cpp/4_CUDA_Libraries/cannyEdgeDetectorNPP`.
> **日本語**
> この sample の目的は、`cannyEdgeDetectorNPP` の小さな実装を通して Runtime, Driver, And NVRTC, CUDA Libraries, Streams And Events, Synchronization And Atomics, Performance を具体的に追うことです。
>
> **学習メモ**
> 最初に `cannyEdgeDetectorNPP.cpp` を読み、setup、GPU work、sync、validation、cleanup の境界を source 内で対応させます。
...
- `cannyEdgeDetectorNPP.cpp`: Host-side setup, API calls, validation, and cleanup.
...
- Open `cannyEdgeDetectorNPP.cpp` first and locate the host-side setup or Python entry point.
...
| `nppiNormL2` | CUDA library call です。handle/descriptor/workspace と data layout を確認します。 |
...
```bash
cmake -S . -B build
cmake --build build --target cannyEdgeDetectorNPP
ctest --test-dir build -R cannyEdgeDetectorNPP
```
