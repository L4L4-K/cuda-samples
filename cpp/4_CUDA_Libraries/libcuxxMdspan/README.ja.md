...
Original README headings: `libcuxxMdspan - libcu++ mdspan Interop (DLPack + shared_memory_mdspan)`, `Description`, `Key Concepts`, `Supported SM Architectures`, `Supported OSes`, `Supported CPU Architecture`, `CUDA APIs involved`, `[CCCL libcu++](https://nvidia.github.io/cccl/libcudacxx/)`

> **日本語**
> `cpp/4_CUDA_Libraries/libcuxxMdspan` は `4_CUDA_Libraries` に属する CUDA sample です。英語 README の目的と手順を残し、ここでは日本語で実装の読み方を補足します。
>
> **学習メモ**
...
English anchor: this sample demonstrates `libcuxxMdspan - libcu++ mdspan Interop (DLPack + shared_memory_mdspan)` and should be read together with the original README and source files.
