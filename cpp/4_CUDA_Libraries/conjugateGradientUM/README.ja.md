# conjugateGradientUM - ConjugateGradientUM - Japanese Learning Guide
...
Original README headings: `conjugateGradientUM - ConjugateGradientUM`, `Description`, `Key Concepts`, `Supported SM Architectures`, `Supported OSes`, `Supported CPU Architecture`, `CUDA APIs involved`, `[CUDA Runtime API](http://docs.nvidia.com/cuda/cuda-runtime-api/index.html)`, `Dependencies needed to build/run`, `Prerequisites`, `References (for more details)`

> **日本語**
> `cpp/4_CUDA_Libraries/conjugateGradientUM` は `C++/CUDA` の CUDA sample です。英語 README を正本として残し、この guide では実装を読む順番と CUDA の学習ポイントを日本語で補足します。
>
> **学習メモ**
...
English anchor: read `conjugateGradientUM` as a focused example of the CUDA concepts used in `cpp/4_CUDA_Libraries/conjugateGradientUM`.
> **日本語**
> この sample の目的は、`conjugateGradientUM` の小さな実装を通して CUDA Libraries, Shared Memory, Unified Memory, Memory, Execution Model を具体的に追うことです。
>
> **学習メモ**
...
- `main.cpp`: focus on `cublasHandle`, `cublasStatus`, `CUDA`, `cudaFree`, `cudaMallocManaged`.
...
Source: cpp/4_CUDA_Libraries/conjugateGradientUM/main.cpp:35-54
```cpp
// includes, system
#include <stdio.h>
#include <stdlib.h>
...
> JP: この抜粋は `cpp/4_CUDA_Libraries/conjugateGradientUM/main.cpp` の実コードです。setup、allocation、transfer、GPU work、sync、validation、cleanup のどの境界を示すかを、行番号と一緒に確認します。

Source: cpp/4_CUDA_Libraries/conjugateGradientUM/main.cpp:120-157
```cpp
...
Source: cpp/4_CUDA_Libraries/conjugateGradientUM/main.cpp:175-194
```cpp
...
Source: cpp/4_CUDA_Libraries/conjugateGradientUM/main.cpp:293-310
```cpp
    }
    if (vecp) {
...
| `cublasStatus` | Driver API の handle 境界です。context/module/function と error code を追います。 |
...
| `cusparseStatus` | Driver API の handle 境界です。context/module/function と error code を追います。 |
...
```bash
cmake -S . -B build
cmake --build build --target conjugateGradientUM
ctest --test-dir build -R conjugateGradientUM
```
...
- `cublasHandle` の直前と直後で、どの memory/resource が有効になったかをメモする。
