# freeImageInteropNPP - FreeImage and NPP Interopability - Japanese Learning Guide
...
Original README headings: `freeImageInteropNPP - FreeImage and NPP Interopability`, `Description`, `Key Concepts`, `Supported SM Architectures`, `Supported OSes`, `Supported CPU Architecture`, `CUDA APIs involved`, `[CUDA Runtime API](http://docs.nvidia.com/cuda/cuda-runtime-api/index.html)`, `Dependencies needed to build/run`, `Prerequisites`, `References (for more details)`

> **日本語**
> `cpp/4_CUDA_Libraries/freeImageInteropNPP` は `C++/CUDA` の CUDA sample です。英語 README を正本として残し、この guide では実装を読む順番と CUDA の学習ポイントを日本語で補足します。
>
> **学習メモ**
...
English anchor: read `freeImageInteropNPP` as a focused example of the CUDA concepts used in `cpp/4_CUDA_Libraries/freeImageInteropNPP`.
> **日本語**
> この sample の目的は、`freeImageInteropNPP` の小さな実装を通して Runtime, Driver, And NVRTC, CUDA Libraries, Streams And Events, Synchronization And Atomics, Performance を具体的に追うことです。
>
> **学習メモ**
> 最初に `freeImageInteropNPP.cpp` を読み、setup、GPU work、sync、validation、cleanup の境界を source 内で対応させます。
...
- `freeImageInteropNPP.cpp`: Host-side setup, API calls, validation, and cleanup.
...
- Open `freeImageInteropNPP.cpp` first and locate the host-side setup or Python entry point.
...
| `nppiMalloc_8u_C1` | device 側 storage を確保する API です。対応する cleanup と byte size を確認します。 |
...
```bash
cmake -S . -B build
cmake --build build --target freeImageInteropNPP
ctest --test-dir build -R freeImageInteropNPP
```
