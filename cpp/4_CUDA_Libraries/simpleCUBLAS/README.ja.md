# simpleCUBLAS - Simple CUBLAS - Japanese Learning Guide
...
Original README headings: `simpleCUBLAS - Simple CUBLAS`, `Description`, `Key Concepts`, `Supported SM Architectures`, `Supported OSes`, `Supported CPU Architecture`, `CUDA APIs involved`, `[CUDA Runtime API](http://docs.nvidia.com/cuda/cuda-runtime-api/index.html)`, `Dependencies needed to build/run`, `Prerequisites`, `References (for more details)`

> **日本語**
> `cpp/4_CUDA_Libraries/simpleCUBLAS` は `C++/CUDA` の CUDA sample です。英語 README を正本として残し、この guide では実装を読む順番と CUDA の学習ポイントを日本語で補足します。
>
> **学習メモ**
...
English anchor: read `simpleCUBLAS` as a focused example of the CUDA concepts used in `cpp/4_CUDA_Libraries/simpleCUBLAS`.
> **日本語**
> この sample の目的は、`simpleCUBLAS` の小さな実装を通して CUDA Libraries, Performance, Memory, Execution Model, Debugging, Profiling, And Testing を具体的に追うことです。
>
> **学習メモ**
> 最初に `simpleCUBLAS.cpp` を読み、setup、GPU work、sync、validation、cleanup の境界を source 内で対応させます。
...
- `simpleCUBLAS.cpp`: Host-side setup, API calls, validation, and cleanup.
...
- Open `simpleCUBLAS.cpp` first and locate the host-side setup or Python entry point.
...
| `cublasSetVector` | Driver API の handle 境界です。context/module/function と error code を追います。 |
| `cublas` | Driver API の handle 境界です。context/module/function と error code を追います。 |
...
| `cublas_v2` | Driver API の handle 境界です。context/module/function と error code を追います。 |
| `cublasHandle_t` | Driver API の handle 境界です。context/module/function と error code を追います。 |
| `cublasCreate` | Driver API の handle 境界です。context/module/function と error code を追います。 |
| `cublasSgemm` | Driver API の handle 境界です。context/module/function と error code を追います。 |
| `cublasGetVector` | Driver API の handle 境界です。context/module/function と error code を追います。 |
| `cublasDestroy` | resource lifetime を閉じる API です。未完了 work が残っていないかを確認します。 |
...
```bash
cmake -S . -B build
cmake --build build --target simpleCUBLAS
ctest --test-dir build -R simpleCUBLAS
```
