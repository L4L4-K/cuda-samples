# boxFilterNPP - Box Filter with NPP - Japanese Learning Guide
...
Original README headings: `boxFilterNPP - Box Filter with NPP`, `Description`, `Key Concepts`, `Supported SM Architectures`, `Supported OSes`, `Supported CPU Architecture`, `CUDA APIs involved`, `[CUDA Runtime API](http://docs.nvidia.com/cuda/cuda-runtime-api/index.html)`, `Dependencies needed to build/run`, `Prerequisites`, `References (for more details)`

> **日本語**
> `cpp/4_CUDA_Libraries/boxFilterNPP` は `C++/CUDA` の CUDA sample です。英語 README を正本として残し、この guide では実装を読む順番と CUDA の学習ポイントを日本語で補足します。
>
> **学習メモ**
...
English anchor: read `boxFilterNPP` as a focused example of the CUDA concepts used in `cpp/4_CUDA_Libraries/boxFilterNPP`.
> **日本語**
> この sample の目的は、`boxFilterNPP` の小さな実装を通して Runtime, Driver, And NVRTC, CUDA Libraries, Streams And Events, Performance, Memory を具体的に追うことです。
>
> **学習メモ**
> 最初に `boxFilterNPP.cpp` を読み、setup、GPU work、sync、validation、cleanup の境界を source 内で対応させます。
...
- `boxFilterNPP.cpp`: Host-side setup, API calls, validation, and cleanup.
- `teapot512.pgm`: Input, reference, generated-data description, or documentation used by the sample.
...
- Open `boxFilterNPP.cpp` first and locate the host-side setup or Python entry point.
...
- `boxFilterNPP.cpp`: focus on `nppStreamCtx`, `CUDA`, `cudaError`, `npp`, `cudaSuccess`.
...
Source: cpp/4_CUDA_Libraries/boxFilterNPP/CMakeLists.txt:1-23
```cmake
...
> JP: この抜粋は `cpp/4_CUDA_Libraries/boxFilterNPP/CMakeLists.txt` の実コードです。setup、allocation、transfer、GPU work、sync、validation、cleanup のどの境界を示すかを、行番号と一緒に確認します。

Source: cpp/4_CUDA_Libraries/boxFilterNPP/CMakeLists.txt:42-61
```cmake
...
Source: cpp/4_CUDA_Libraries/boxFilterNPP/boxFilterNPP.cpp:29-70
```cpp
...
> JP: この抜粋は `cpp/4_CUDA_Libraries/boxFilterNPP/boxFilterNPP.cpp` の実コードです。setup、allocation、transfer、GPU work、sync、validation、cleanup のどの境界を示すかを、行番号と一緒に確認します。

Source: cpp/4_CUDA_Libraries/boxFilterNPP/boxFilterNPP.cpp:173-192
```cpp
...
| `cudaStreamGetFlags` | 非同期 work の順序、overlap、計測範囲を表す API です。 |
...
| `nppGetLibVersion` | CUDA library call です。handle/descriptor/workspace と data layout を確認します。 |
| `cudaDevAttrComputeCapabilityMajor` | この sample の中心 API/概念です。入力、所有権、同期、検証との関係を確認します。 |
| `cudaDevAttrComputeCapabilityMinor` | この sample の中心 API/概念です。入力、所有権、同期、検証との関係を確認します。 |
...
```bash
cmake -S . -B build
cmake --build build --target boxFilterNPP
ctest --test-dir build -R boxFilterNPP
```
