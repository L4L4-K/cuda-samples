...
Original README headings: `simpleCUFFT_MGPU - Simple CUFFT_MGPU`, `Description`, `Key Concepts`, `Supported SM Architectures`, `Supported OSes`, `Supported CPU Architecture`, `CUDA APIs involved`, `[CUDA Runtime API](http://docs.nvidia.com/cuda/cuda-runtime-api/index.html)`, `Dependencies needed to build/run`, `Prerequisites`, `References (for more details)`

> **日本語**
> `cpp/4_CUDA_Libraries/simpleCUFFT_MGPU` は `C++/CUDA` の CUDA sample です。英語 README を正本として残し、この guide では実装を読む順番と CUDA の学習ポイントを日本語で補足します。
>
> **学習メモ**
...
English anchor: read `simpleCUFFT_MGPU` as a focused example of the CUDA concepts used in `cpp/4_CUDA_Libraries/simpleCUFFT_MGPU`.
> **日本語**
> この sample の目的は、`simpleCUFFT_MGPU` の小さな実装を通して CUDA Libraries, Memory, Kernel Launch And Indexing, Execution Model, Debugging, Profiling, And Testing を具体的に追うことです。
>
> **学習メモ**
> 最初に `simpleCUFFT_MGPU.cu` を読み、setup、GPU work、sync、validation、cleanup の境界を source 内で対応させます。
...
- `simpleCUFFT_MGPU.cu`: CUDA source containing kernels, Runtime API calls, or device-side helper code.
...
- Open `simpleCUFFT_MGPU.cu` first and locate the host-side setup or Python entry point.
...
| `CUFFT_XT_FORMAT_INPLACE` | Driver API の handle 境界です。context/module/function と error code を追います。 |
...
ctest --test-dir build -R simpleCUFFT_MGPU
```
...
- `cudaLibXtDesc` の直前と直後で、どの memory/resource が有効になったかをメモする。
