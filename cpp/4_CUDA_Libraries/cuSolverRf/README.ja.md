# cuSolverRf - cuSolverRf Refactorization - Japanese Learning Guide
...
Original README headings: `cuSolverRf - cuSolverRf Refactorization`, `Description`, `Key Concepts`, `Supported SM Architectures`, `Supported OSes`, `Supported CPU Architecture`, `CUDA APIs involved`, `[CUDA Driver API](http://docs.nvidia.com/cuda/cuda-driver-api/index.html)`, `[CUDA Runtime API](http://docs.nvidia.com/cuda/cuda-runtime-api/index.html)`, `Dependencies needed to build/run`, `Prerequisites`, `References (for more details)`

> **日本語**
> `cpp/4_CUDA_Libraries/cuSolverRf` は `C++/CUDA` の CUDA sample です。英語 README を正本として残し、この guide では実装を読む順番と CUDA の学習ポイントを日本語で補足します。
>
> **学習メモ**
...
English anchor: read `cuSolverRf` as a focused example of the CUDA concepts used in `cpp/4_CUDA_Libraries/cuSolverRf`.
> **日本語**
> この sample の目的は、`cuSolverRf` の小さな実装を通して Runtime, Driver, And NVRTC, CUDA Libraries, Shared Memory, Streams And Events, Memory を具体的に追うことです。
>
> **学習メモ**
> 最初に `cuSolverRf.cpp, mmio.c, mmio.h, mmio_wrapper.cpp` を読み、setup、GPU work、sync、validation、cleanup の境界を source 内で対応させます。
...
- `cuSolverRf.cpp`: Host-side setup, API calls, validation, and cleanup.
...
- Open `cuSolverRf.cpp` first and locate the host-side setup or Python entry point.
...
- `cuSolverRf.cpp`: focus on `CUDA`, `cusolverSpH`, `cudaMemcpy`, `cusolverRf`, `cusolverRfH`.
...
Source: cpp/4_CUDA_Libraries/cuSolverRf/CMakeLists.txt:1-62
```cmake
...
# JP: `cuSolverRf`: Driver API は CU* handle を明示的に扱います。context/module/function の所有と error check を追います。 CUDA library の handle/descriptor/workspace は外部 resource です。作成、設定、利用、破棄の順序を対応させます。
...
> JP: この抜粋は `cpp/4_CUDA_Libraries/cuSolverRf/CMakeLists.txt` の実コードです。setup、allocation、transfer、GPU work、sync、validation、cleanup のどの境界を示すかを、行番号と一緒に確認します。

### `cuSolverRf.cpp`

Source: cpp/4_CUDA_Libraries/cuSolverRf/cuSolverRf.cpp:43-64
```cpp
 *
 */
...
> JP: この抜粋は `cpp/4_CUDA_Libraries/cuSolverRf/cuSolverRf.cpp` の実コードです。setup、allocation、transfer、GPU work、sync、validation、cleanup のどの境界を示すかを、行番号と一緒に確認します。

Source: cpp/4_CUDA_Libraries/cuSolverRf/cuSolverRf.cpp:122-144
```cpp
            UsageRF();
        }
    }
...
int main(int argc, char *argv[])
{
    ...
    // JP: `cusolverRfHandle_t`, `cusolverRfH`: CUDA library の handle/descriptor/workspace は外部 resource です。作成、設定、利用、破棄の順序を対応させます。
...
Source: cpp/4_CUDA_Libraries/cuSolverRf/cuSolverRf.cpp:295-316
```cpp
    }
...
Source: cpp/4_CUDA_Libraries/cuSolverRf/cuSolverRf.cpp:491-510
```cpp
    stop          = second();
...
Source: cpp/4_CUDA_Libraries/cuSolverRf/mmio.c:11-29
```c
#if defined(_WIN32)
...
> JP: この抜粋は `cpp/4_CUDA_Libraries/cuSolverRf/mmio.c` の実コードです。setup、allocation、transfer、GPU work、sync、validation、cleanup のどの境界を示すかを、行番号と一緒に確認します。

Source: cpp/4_CUDA_Libraries/cuSolverRf/mmio.c:56-75
```c
    *N_  = N;
    *nz_ = nz;
...
Source: cpp/4_CUDA_Libraries/cuSolverRf/mmio.c:362-381
```c
...
Source: cpp/4_CUDA_Libraries/cuSolverRf/mmio.h:10-28
```cpp
#ifndef MM_IO_H
#define MM_IO_H
...
> JP: この抜粋は `cpp/4_CUDA_Libraries/cuSolverRf/mmio.h` の実コードです。setup、allocation、transfer、GPU work、sync、validation、cleanup のどの境界を示すかを、行番号と一緒に確認します。

### `mmio_wrapper.cpp`

Source: cpp/4_CUDA_Libraries/cuSolverRf/mmio_wrapper.cpp:29-47
```cpp
...
> JP: この抜粋は `cpp/4_CUDA_Libraries/cuSolverRf/mmio_wrapper.cpp` の実コードです。setup、allocation、transfer、GPU work、sync、validation、cleanup のどの境界を示すかを、行番号と一緒に確認します。

Source: cpp/4_CUDA_Libraries/cuSolverRf/mmio_wrapper.cpp:282-301
```cpp
...
Source: cpp/4_CUDA_Libraries/cuSolverRf/mmio_wrapper.cpp:323-342
```cpp
            }
        }
        (*nnz) += count;
...
Source: cpp/4_CUDA_Libraries/cuSolverRf/mmio_wrapper.cpp:411-430
```cpp
...
| `cusolverRf` | Driver API の handle 境界です。context/module/function と error code を追います。 |
| `cusolverRfH` | Driver API の handle 境界です。context/module/function と error code を追います。 |
| `cudaMalloc` | device 側 storage を確保する API です。対応する cleanup と byte size を確認します。 |
| `cudaMemcpyHostToDevice` | host/device 間の転送、初期化、または visibility を作る API です。方向と Async の順序を確認します。 |
| `cudaFree` | resource lifetime を閉じる API です。未完了 work が残っていないかを確認します。 |
| `cusolverSp` | Driver API の handle 境界です。context/module/function と error code を追います。 |
...
```bash
cmake -S . -B build
cmake --build build --target cuSolverRf
ctest --test-dir build -R cuSolverRf
```
