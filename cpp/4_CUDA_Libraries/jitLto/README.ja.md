# jitLto - Saxpy with libnvJitLink - Japanese Learning Guide
...
Original README headings: `jitLto - Saxpy with libnvJitLink`, `Description`, `Key Concepts`, `Supported SM Architectures`, `Supported OSes`, `Supported CPU Architecture`, `CUDA APIs involved`, `[CUDA Driver API](http://docs.nvidia.com/cuda/cuda-driver-api/index.html)`, `Dependencies needed to build/run`, `Prerequisites`, `References (for more details)`

> **日本語**
> `cpp/4_CUDA_Libraries/jitLto` は `C++/CUDA` の CUDA sample です。英語 README を正本として残し、この guide では実装を読む順番と CUDA の学習ポイントを日本語で補足します。
>
> **学習メモ**
...
English anchor: read `jitLto` as a focused example of the CUDA concepts used in `cpp/4_CUDA_Libraries/jitLto`.
> **日本語**
> この sample の目的は、`jitLto` の小さな実装を通して Runtime, Driver, And NVRTC, Multi-GPU, P2P, And IPC, Performance, Memory, Kernel Launch And Indexing を具体的に追うことです。
>
> **学習メモ**
> 最初に `jitLto.cpp` を読み、setup、GPU work、sync、validation、cleanup の境界を source 内で対応させます。
...
- `jitLto.cpp`: Host-side setup, API calls, validation, and cleanup.
...
- Open `jitLto.cpp` first and locate the host-side setup or Python entry point.
...
- `jitLto.cpp`: focus on `CUDA_SAFE_CALL`, `cuDevice`, `cuMemAlloc`, `launch`, `nvrtc`.
...
| - | - |
| `CUDA_SAFE_CALL` | この sample の中心 API/概念です。入力、所有権、同期、検証との関係を確認します。 |
...
| `nvJitLink` | 実行時 compile/link の境界です。log、module、kernel name の対応を確認します。 |
| `nvrtcProgram` | 実行時 compile/link の境界です。log、module、kernel name の対応を確認します。 |
...
| `nvJitLinkResult` | 実行時 compile/link の境界です。log、module、kernel name の対応を確認します。 |
...
```bash
cmake -S . -B build
cmake --build build --target jitLto
ctest --test-dir build -R jitLto
```
...
- `CUDA_SAFE_CALL` の直前と直後で、どの memory/resource が有効になったかをメモする。
