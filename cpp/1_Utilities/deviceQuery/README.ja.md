# deviceQuery - Device Query - Japanese Learning Guide
...
Original README headings: `deviceQuery - Device Query`, `Description`, `Key Concepts`, `Supported SM Architectures`, `Supported OSes`, `Supported CPU Architecture`, `CUDA APIs involved`, `[CUDA Driver API](http://docs.nvidia.com/cuda/cuda-driver-api/index.html)`, `[CUDA Runtime API](http://docs.nvidia.com/cuda/cuda-runtime-api/index.html)`, `Prerequisites`, `References (for more details)`

> **日本語**
> `cpp/1_Utilities/deviceQuery` は `C++/CUDA` の CUDA sample です。英語 README を正本として残し、この guide では実装を読む順番と CUDA の学習ポイントを日本語で補足します。
>
> **学習メモ**
...
English anchor: read `deviceQuery` as a focused example of the CUDA concepts used in `cpp/1_Utilities/deviceQuery`.
> **日本語**
> この sample の目的は、`deviceQuery` の小さな実装を通して Runtime, Driver, And NVRTC, Multi-GPU, P2P, And IPC, Streams And Events, Synchronization And Atomics, Memory を具体的に追うことです。
>
> **学習メモ**
> 最初に `deviceQuery.cpp` を読み、setup、GPU work、sync、validation、cleanup の境界を source 内で対応させます。
...
- `deviceQuery.cpp`: Host-side setup, API calls, validation, and cleanup.
...
- `deviceQuery.cpp`: focus on `CUDA`, `Device`, `CUDART_VERSION`, `cudaSetDevice`, `cudaDeviceGetAttribute`.
...
```bash
cmake -S . -B build
cmake --build build --target deviceQuery
ctest --test-dir build -R deviceQuery
```
