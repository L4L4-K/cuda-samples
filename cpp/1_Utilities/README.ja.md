# 1. Utilities - Japanese Learning Guide
...
Original README headings: `1. Utilities`, `[deviceQuery](./deviceQuery)`, `[deviceQueryDrv](./deviceQueryDrv)`, `[topologyQuery](./topologyQuery)`, `Note`, `bandwidthTest`

> **日本語**
> `cpp/1_Utilities` は `C++/CUDA` の CUDA sample です。英語 README を正本として残し、この guide では実装を読む順番と CUDA の学習ポイントを日本語で補足します。
>
> **学習メモ**
...
English anchor: read `1_Utilities` as a focused example of the CUDA concepts used in `cpp/1_Utilities`.
> **日本語**
> この sample の目的は、`1_Utilities` の小さな実装を通して Runtime, Driver, And NVRTC, Multi-GPU, P2P, And IPC, Cooperative Groups, Streams And Events, Unified Memory を具体的に追うことです。
>
> **学習メモ**
> 最初に `deviceQuery.cpp, deviceQueryDrv.cpp, topologyQuery.cu` を読み、setup、GPU work、sync、validation、cleanup の境界を source 内で対応させます。
...
- `deviceQuery/.vscode/c_cpp_properties.json`: Editor configuration. It is useful for navigation, but not part of CUDA runtime behavior.
- `deviceQuery/.vscode/extensions.json`: Editor configuration. It is useful for navigation, but not part of CUDA runtime behavior.
- `deviceQuery/CMakeLists.txt`: CMake target, CUDA architecture, and library dependency wiring.
- `deviceQuery/README.md`: Original English purpose, prerequisites, run notes, and expected behavior.
- `deviceQuery/deviceQuery.cpp`: Host-side setup, API calls, validation, and cleanup.
- `deviceQueryDrv/.vscode/c_cpp_properties.json`: Editor configuration. It is useful for navigation, but not part of CUDA runtime behavior.
- `deviceQueryDrv/.vscode/extensions.json`: Editor configuration. It is useful for navigation, but not part of CUDA runtime behavior.
- `deviceQueryDrv/CMakeLists.txt`: CMake target, CUDA architecture, and library dependency wiring.
- `deviceQueryDrv/README.md`: Original English purpose, prerequisites, run notes, and expected behavior.
- `deviceQueryDrv/deviceQueryDrv.cpp`: Host-side setup, API calls, validation, and cleanup.
- `topologyQuery/.vscode/c_cpp_properties.json`: Editor configuration. It is useful for navigation, but not part of CUDA runtime behavior.
- `topologyQuery/.vscode/extensions.json`: Editor configuration. It is useful for navigation, but not part of CUDA runtime behavior.
- `topologyQuery/CMakeLists.txt`: CMake target, CUDA architecture, and library dependency wiring.
- `topologyQuery/README.md`: Original English purpose, prerequisites, run notes, and expected behavior.
- `topologyQuery/topologyQuery.cu`: CUDA source containing kernels, Runtime API calls, or device-side helper code.
...
- Open `deviceQuery.cpp` first and locate the host-side setup or Python entry point.
...
- `deviceQuery/deviceQuery.cpp`: focus on `CUDA`, `Device`, `CUDART_VERSION`, `cudaSetDevice`, `cudaDeviceGetAttribute`.
- `deviceQueryDrv/deviceQueryDrv.cpp`: focus on `CUDA`, `Device`, `blockDim`, `gridDim`, `cuDeviceGetAttribute`.
- `topologyQuery/topologyQuery.cu`: focus on `atomicSupported`, `Device`, `cudaDeviceGetP2PAttribute`, `CUDA`, `atomic`.
...
```bash
cmake -S . -B build
cmake --build build --target 1_Utilities
ctest --test-dir build -R 1_Utilities
```
