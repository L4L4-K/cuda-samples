...
Original README headings: `deviceQueryDrv - Device Query Driver API`, `Description`, `Key Concepts`, `Supported SM Architectures`, `Supported OSes`, `Supported CPU Architecture`, `CUDA APIs involved`, `[CUDA Driver API](http://docs.nvidia.com/cuda/cuda-driver-api/index.html)`

> **日本語**
> `cpp/1_Utilities/deviceQueryDrv` は `1_Utilities` に属する CUDA sample です。英語 README の目的と手順を残し、ここでは日本語で実装の読み方を補足します。
>
> **学習メモ**
...
English anchor: this sample demonstrates `deviceQueryDrv - Device Query Driver API` and should be read together with the original README and source files.
