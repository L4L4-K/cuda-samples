# EGLSync_CUDAEvent_Interop - EGLSync CUDA Event Interop - Japanese Learning Guide
...
Original README headings: `EGLSync_CUDAEvent_Interop - EGLSync CUDA Event Interop`, `Description`, `Key Concepts`, `Supported SM Architectures`, `Supported OSes`, `Supported CPU Architecture`, `CUDA APIs involved`, `[CUDA Driver API](http://docs.nvidia.com/cuda/cuda-driver-api/index.html)`, `[CUDA Runtime API](http://docs.nvidia.com/cuda/cuda-runtime-api/index.html)`, `Dependencies needed to build/run`, `Prerequisites`, `References (for more details)`

> **日本語**
> `cpp/8_Platform_Specific/Tegra/EGLSync_CUDAEvent_Interop` は `C++/CUDA` の CUDA sample です。英語 README を正本として残し、この guide では実装を読む順番と CUDA の学習ポイントを日本語で補足します。
>
> **学習メモ**
...
English anchor: read `EGLSync_CUDAEvent_Interop` as a focused example of the CUDA concepts used in `cpp/8_Platform_Specific/Tegra/EGLSync_CUDAEvent_Interop`.
> **日本語**
> この sample の目的は、`EGLSync_CUDAEvent_Interop` の小さな実装を通して Runtime, Driver, And NVRTC, CUDA Graphs, Streams And Events, Synchronization And Atomics, Performance を具体的に追うことです。
>
> **学習メモ**
> 最初に `EGLSync_CUDAEvent_Interop.cu, egl_common.h, graphics_interface.h` を読み、setup、GPU work、sync、validation、cleanup の境界を source 内で対応させます。
...
- `EGLSync_CUDAEvent_Interop.cu`: CUDA source containing kernels, Runtime API calls, or device-side helper code.
- `README.md`: Original English purpose, prerequisites, run notes, and expected behavior.
- `egl_common.h`: Host/device declarations, helper types, constants, or library wrappers.
...
- `EGLSync_CUDAEvent_Interop.cu`: focus on `CUDA_SUCCESS`, `CUDA`, `cudaSuccess`, `CUsurfObject`, `cudaGetErrorString`.
- `egl_common.h`: focus on `cudaEGL`.
...
| `Event` | 非同期 work の順序、overlap、計測範囲を表す API です。 |
| `cudaGetErrorString` | この sample の中心 API/概念です。入力、所有権、同期、検証との関係を確認します。 |
| `CUsurfObject` | Driver API の handle 境界です。context/module/function と error code を追います。 |
| `cuGraphicsEGLRegisterImage` | CUDA Graph の node、capture、instantiate、launch、update の境界を表します。 |
| `cudaGetValueMismatch` | この sample の中心 API/概念です。入力、所有権、同期、検証との関係を確認します。 |
| `cuGraphicsSubResourceGetMappedArray` | CUDA Graph の node、capture、instantiate、launch、update の境界を表します。 |
| `cuSurfObjectCreate` | Driver API の handle 境界です。context/module/function と error code を追います。 |
| `cuGraphicsUnregisterResource` | CUDA Graph の node、capture、instantiate、launch、update の境界を表します。 |
| `cuStreamWaitEvent` | 非同期 work の順序、overlap、計測範囲を表す API です。 |
...
```bash
cmake -S . -B build
cmake --build build --target EGLSync_CUDAEvent_Interop
ctest --test-dir build -R EGLSync_CUDAEvent_Interop
```
