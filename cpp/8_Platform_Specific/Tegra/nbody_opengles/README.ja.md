# nbody_opengles - CUDA N-Body Simulation with GLES - Japanese Learning Guide
...
Original README headings: `nbody_opengles - CUDA N-Body Simulation with GLES`, `Description`, `Key Concepts`, `Supported SM Architectures`, `Supported OSes`, `Supported CPU Architecture`, `CUDA APIs involved`, `[CUDA Runtime API](http://docs.nvidia.com/cuda/cuda-runtime-api/index.html)`, `Dependencies needed to build/run`, `Prerequisites`, `References (for more details)`

> **日本語**
> `cpp/8_Platform_Specific/Tegra/nbody_opengles` は `C++/CUDA` の CUDA sample です。英語 README を正本として残し、この guide では実装を読む順番と CUDA の学習ポイントを日本語で補足します。
>
> **学習メモ**
...
English anchor: read `nbody_opengles` as a focused example of the CUDA concepts used in `cpp/8_Platform_Specific/Tegra/nbody_opengles`.
> **日本語**
> この sample の目的は、`nbody_opengles` の小さな実装を通して CUDA Graphs, Shared Memory, Streams And Events, Synchronization And Atomics, Performance を具体的に追うことです。
>
> **学習メモ**
...
- `galaxy_20K.bin`: Input, reference, generated-data description, or documentation used by the sample.
- `nbody_opengles.cpp`: Host-side setup, API calls, validation, and cleanup.
...
```bash
cmake -S . -B build
cmake --build build --target nbody_opengles
ctest --test-dir build -R nbody_opengles
```
