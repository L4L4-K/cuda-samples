...
Original README headings: `cuDLALayerwiseStatsStandalone - cuDLA Layerwise Statistics Standalone Mode`, `Description`, `Key Concepts`, `Supported SM Architectures`, `Supported OSes`, `Supported CPU Architecture`, `CUDA APIs involved`, `Dependencies needed to build/run`

> **日本語**
> `cpp/8_Platform_Specific/Tegra/cuDLALayerwiseStatsStandalone` は `8_Platform_Specific` に属する CUDA sample です。英語 README の目的と手順を残し、ここでは日本語で実装の読み方を補足します。
>
> **学習メモ**
...
English anchor: this sample demonstrates `cuDLALayerwiseStatsStandalone - cuDLA Layerwise Statistics Standalone Mode` and should be read together with the original README and source files.
