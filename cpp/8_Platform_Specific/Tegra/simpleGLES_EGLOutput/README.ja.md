# simpleGLES_EGLOutput - Simple OpenGLES EGLOutput - Japanese Learning Guide
...
Original README headings: `simpleGLES_EGLOutput - Simple OpenGLES EGLOutput`, `Description`, `Key Concepts`, `Supported SM Architectures`, `Supported OSes`, `Supported CPU Architecture`, `CUDA APIs involved`, `[CUDA Runtime API](http://docs.nvidia.com/cuda/cuda-runtime-api/index.html)`, `Dependencies needed to build/run`, `Prerequisites`, `References (for more details)`

> **日本語**
> `cpp/8_Platform_Specific/Tegra/simpleGLES_EGLOutput` は `C++/CUDA` の CUDA sample です。英語 README を正本として残し、この guide では実装を読む順番と CUDA の学習ポイントを日本語で補足します。
>
> **学習メモ**
...
English anchor: read `simpleGLES_EGLOutput` as a focused example of the CUDA concepts used in `cpp/8_Platform_Specific/Tegra/simpleGLES_EGLOutput`.
> **日本語**
> この sample の目的は、`simpleGLES_EGLOutput` の小さな実装を通して CUDA Graphs, Streams And Events, Synchronization And Atomics, Memory, Kernel Launch And Indexing を具体的に追うことです。
>
> **学習メモ**
> 最初に `graphics_interface_egloutput_via_egl.c, simpleGLES_EGLOutput.cu` を読み、setup、GPU work、sync、validation、cleanup の境界を source 内で対応させます。
...
- `data/ref_simpleGLES_EGLOutput.bin`: Input, reference, generated-data description, or documentation used by the sample.
- `graphics_interface_egloutput_via_egl.c`: Host-side setup, API calls, validation, and cleanup.
- `mesh.frag.glsl`: Supporting file used by `mesh.frag.glsl`.
- `mesh.vert.glsl`: Supporting file used by `mesh.vert.glsl`.
- `simpleGLES_EGLOutput.cu`: CUDA source containing kernels, Runtime API calls, or device-side helper code.
...
- Open `graphics_interface_egloutput_via_egl.c` first and locate the host-side setup or Python entry point.
...
```bash
cmake -S . -B build
cmake --build build --target simpleGLES_EGLOutput
ctest --test-dir build -R simpleGLES_EGLOutput
```
