# quasirandomGenerator_nvrtc - Niederreiter Quasirandom Sequence Generator with libNVRTC - Japanese Learning Guide
...
English anchor: read `quasirandomGenerator_nvrtc` as a focused example of the CUDA concepts used in `cpp/5_Domain_Specific/quasirandomGenerator_nvrtc`.
> **日本語**
> この sample の目的は、`quasirandomGenerator_nvrtc` の小さな実装を通して Runtime, Driver, And NVRTC, Multi-GPU, P2P, And IPC, Streams And Events, Synchronization And Atomics, Memory を具体的に追うことです。
>
> **学習メモ**
> 最初に `quasirandomGenerator.cpp, quasirandomGenerator_common.h, quasirandomGenerator_gold.cpp, quasirandomGenerator_gpu.cuh, quasirandomGenerator_kernel.cu` を読み、setup、GPU work、sync、validation、cleanup の境界を source 内で対応させます。
...
- `quasirandomGenerator_gpu.cuh`: CUDA header with device functions, kernels, templates, or shared constants.
...
- `quasirandomGenerator.cpp`: focus on `cuMemcpyDtoH`, `cuMemAlloc`, `cuMemsetD8`, `CUDA`, `CUdeviceptr`.
...
- `quasirandomGenerator_gpu.cuh`: focus on `cudaGridSize`, `CUdeviceptr`, `cuLaunchKernel`, `cuMemcpyHtoD`, `CUfunction`.
- `quasirandomGenerator_kernel.cu`: focus on `threadIdx`, `blockDim`, `blockIdx`, `gridDim`, `launch`.
...
```bash
cmake -S . -B build
cmake --build build --target quasirandomGenerator_nvrtc
ctest --test-dir build -R quasirandomGenerator_nvrtc
```
...
- `cudaGridSize` の直前と直後で、どの memory/resource が有効になったかをメモする。
