# postProcessGL - Post-Process in OpenGL - Japanese Learning Guide
...
Original README headings: `postProcessGL - Post-Process in OpenGL`, `Description`, `Key Concepts`, `Supported SM Architectures`, `Supported OSes`, `Supported CPU Architecture`, `CUDA APIs involved`, `[CUDA Runtime API](http://docs.nvidia.com/cuda/cuda-runtime-api/index.html)`, `Dependencies needed to build/run`, `Prerequisites`, `References (for more details)`

> **日本語**
> `cpp/5_Domain_Specific/postProcessGL` は `C++/CUDA` の CUDA sample です。英語 README を正本として残し、この guide では実装を読む順番と CUDA の学習ポイントを日本語で補足します。
>
> **学習メモ**
...
English anchor: read `postProcessGL` as a focused example of the CUDA concepts used in `cpp/5_Domain_Specific/postProcessGL`.
> **日本語**
> この sample の目的は、`postProcessGL` の小さな実装を通して CUDA Graphs, Shared Memory, Streams And Events, Synchronization And Atomics, Memory を具体的に追うことです。
>
> **学習メモ**
> 最初に `main.cpp, postProcessGL.cu` を読み、setup、GPU work、sync、validation、cleanup の境界を source 内で対応させます。
...
- `data/teapot_2.ppm`: Input, reference, generated-data description, or documentation used by the sample.
- `data/teapot_4.ppm`: Input, reference, generated-data description, or documentation used by the sample.
- `data/teapot_8.ppm`: Input, reference, generated-data description, or documentation used by the sample.
- `data/teapot_orig.ppm`: Input, reference, generated-data description, or documentation used by the sample.
- `doc/postProcessGL_lg.gif`: Supporting file used by `doc/postProcessGL_lg.gif`.
- `doc/postProcessGL_md.gif`: Supporting file used by `doc/postProcessGL_md.gif`.
- `doc/postProcessGL_sm.gif`: Supporting file used by `doc/postProcessGL_sm.gif`.
- `main.cpp`: Host-side setup, API calls, validation, and cleanup.
- `postProcessGL.cu`: CUDA source containing kernels, Runtime API calls, or device-side helper code.
...
```bash
cmake -S . -B build
cmake --build build --target postProcessGL
ctest --test-dir build -R postProcessGL
```
