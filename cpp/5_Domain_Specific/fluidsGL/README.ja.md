# fluidsGL - Fluids (OpenGL Version) - Japanese Learning Guide
...
Original README headings: `fluidsGL - Fluids (OpenGL Version)`, `Description`, `Key Concepts`, `Supported SM Architectures`, `Supported OSes`, `Supported CPU Architecture`, `CUDA APIs involved`, `[CUDA Runtime API](http://docs.nvidia.com/cuda/cuda-runtime-api/index.html)`, `Dependencies needed to build/run`, `Prerequisites`, `References (for more details)`

> **日本語**
> `cpp/5_Domain_Specific/fluidsGL` は `C++/CUDA` の CUDA sample です。英語 README を正本として残し、この guide では実装を読む順番と CUDA の学習ポイントを日本語で補足します。
>
> **学習メモ**
...
English anchor: read `fluidsGL` as a focused example of the CUDA concepts used in `cpp/5_Domain_Specific/fluidsGL`.
> **日本語**
> この sample の目的は、`fluidsGL` の小さな実装を通して CUDA Libraries, CUDA Graphs, Streams And Events, Synchronization And Atomics, Memory を具体的に追うことです。
>
> **学習メモ**
> 最初に `defines.h, fluidsGL.cpp, fluidsGL_kernels.cu, fluidsGL_kernels.cuh, fluidsGL_kernels.h` を読み、setup、GPU work、sync、validation、cleanup の境界を source 内で対応させます。
...
- `data/ref_fluidsGL.ppm`: Input, reference, generated-data description, or documentation used by the sample.
- `defines.h`: Host/device declarations, helper types, constants, or library wrappers.
- `doc/fluidsGL.doc`: Supporting file used by `doc/fluidsGL.doc`.
- `doc/fluidsGL.pdf`: Supporting file used by `doc/fluidsGL.pdf`.
- `doc/fluidsGL_lg.gif`: Supporting file used by `doc/fluidsGL_lg.gif`.
- `doc/fluidsGL_md.gif`: Supporting file used by `doc/fluidsGL_md.gif`.
- `doc/fluidsGL_sm.gif`: Supporting file used by `doc/fluidsGL_sm.gif`.
- `fluidsGL.cpp`: Host-side setup, API calls, validation, and cleanup.
- `fluidsGL_kernels.cu`: CUDA source containing kernels, Runtime API calls, or device-side helper code.
- `fluidsGL_kernels.cuh`: CUDA header with device functions, kernels, templates, or shared constants.
- `fluidsGL_kernels.h`: Host/device declarations, helper types, constants, or library wrappers.
...
- `fluidsGL.cpp`: focus on `CUDA`, `cudaGraphicsGLRegisterBuffer`, `cudaFree`, `CUFFT`, `cufftHandle`.
- `fluidsGL_kernels.cu`: focus on `threadIdx`, `CUDA`, `launch`, `blockIdx`, `blockDim`.
- `fluidsGL_kernels.cuh`: focus on `cudaTextureObject_t`.
- `fluidsGL_kernels.h`: focus on `cudaTextureObject_t`.
...
Source: cpp/5_Domain_Specific/fluidsGL/CMakeLists.txt:1-23
```cmake
...
> JP: この抜粋は `cpp/5_Domain_Specific/fluidsGL/CMakeLists.txt` の実コードです。setup、allocation、transfer、GPU work、sync、validation、cleanup のどの境界を示すかを、行番号と一緒に確認します。

Source: cpp/5_Domain_Specific/fluidsGL/CMakeLists.txt:55-74
```cmake
        )
...
Source: cpp/5_Domain_Specific/fluidsGL/defines.h:29-48
```cpp
#ifndef DEFINES_H
#define DEFINES_H
...
> JP: この抜粋は `cpp/5_Domain_Specific/fluidsGL/defines.h` の実コードです。setup、allocation、transfer、GPU work、sync、validation、cleanup のどの境界を示すかを、行番号と一緒に確認します。

### `fluidsGL.cpp`

Source: cpp/5_Domain_Specific/fluidsGL/fluidsGL.cpp:30-48
```cpp
#include <helper_gl.h>
...
> JP: この抜粋は `cpp/5_Domain_Specific/fluidsGL/fluidsGL.cpp` の実コードです。setup、allocation、transfer、GPU work、sync、validation、cleanup のどの境界を示すかを、行番号と一緒に確認します。

Source: cpp/5_Domain_Specific/fluidsGL/fluidsGL.cpp:72-91
```cpp
...
Source: cpp/5_Domain_Specific/fluidsGL/fluidsGL.cpp:214-233
```cpp
...
Source: cpp/5_Domain_Specific/fluidsGL/fluidsGL.cpp:273-292
```cpp
#endif
        break;
...
Source: cpp/5_Domain_Specific/fluidsGL/fluidsGL_kernels.cu:29-67
```cuda
...
> JP: この抜粋は `cpp/5_Domain_Specific/fluidsGL/fluidsGL_kernels.cu` の実コードです。setup、allocation、transfer、GPU work、sync、validation、cleanup のどの境界を示すかを、行番号と一緒に確認します。

Source: cpp/5_Domain_Specific/fluidsGL/fluidsGL_kernels.cu:81-123
```cuda
...
Source: cpp/5_Domain_Specific/fluidsGL/fluidsGL_kernels.cuh:29-72
```cuda
...
> JP: この抜粋は `cpp/5_Domain_Specific/fluidsGL/fluidsGL_kernels.cuh` の実コードです。setup、allocation、transfer、GPU work、sync、validation、cleanup のどの境界を示すかを、行番号と一緒に確認します。

### `fluidsGL_kernels.h`

Source: cpp/5_Domain_Specific/fluidsGL/fluidsGL_kernels.h:29-70
```cpp
#ifndef __STABLEFLUIDS_KERNELS_CUH_
#define __STABLEFLUIDS_KERNELS_CUH_
...
> JP: この抜粋は `cpp/5_Domain_Specific/fluidsGL/fluidsGL_kernels.h` の実コードです。setup、allocation、transfer、GPU work、sync、validation、cleanup のどの境界を示すかを、行番号と一緒に確認します。
...
```bash
cmake -S . -B build
cmake --build build --target fluidsGL
ctest --test-dir build -R fluidsGL
```
