# p2pBandwidthLatencyTest - Peer-to-Peer Bandwidth Latency Test with Multi-GPUs - Japanese Learning Guide
...
Original README headings: `p2pBandwidthLatencyTest - Peer-to-Peer Bandwidth Latency Test with Multi-GPUs`, `Description`, `Key Concepts`, `Supported SM Architectures`, `Supported OSes`, `Supported CPU Architecture`, `CUDA APIs involved`, `[CUDA Runtime API](http://docs.nvidia.com/cuda/cuda-runtime-api/index.html)`, `Prerequisites`, `References (for more details)`

> **日本語**
> `cpp/5_Domain_Specific/p2pBandwidthLatencyTest` は `C++/CUDA` の CUDA sample です。英語 README を正本として残し、この guide では実装を読む順番と CUDA の学習ポイントを日本語で補足します。
>
> **学習メモ**
...
English anchor: read `p2pBandwidthLatencyTest` as a focused example of the CUDA concepts used in `cpp/5_Domain_Specific/p2pBandwidthLatencyTest`.
> **日本語**
> この sample の目的は、`p2pBandwidthLatencyTest` の小さな実装を通して Multi-GPU, P2P, And IPC, Streams And Events, Unified Memory, Performance, Memory を具体的に追うことです。
>
> **学習メモ**
> 最初に `p2pBandwidthLatencyTest.cu` を読み、setup、GPU work、sync、validation、cleanup の境界を source 内で対応させます。
...
- `p2pBandwidthLatencyTest.cu`: CUDA source containing kernels, Runtime API calls, or device-side helper code.
...
- Open `p2pBandwidthLatencyTest.cu` first and locate the host-side setup or Python entry point.
...
- `p2pBandwidthLatencyTest.cu`: focus on `cudaCheckError`, `cudaSetDevice`, `cudaStreamSynchronize`, `cudaMalloc`, `cudaFree`.
...
| `cudaMemcpyPeerAsync` | host/device 間の転送、初期化、または visibility を作る API です。方向と Async の順序を確認します。 |
...
```bash
cmake -S . -B build
cmake --build build --target p2pBandwidthLatencyTest
ctest --test-dir build -R p2pBandwidthLatencyTest
```
...
- `cudaCheckError` の直前と直後で、どの memory/resource が有効になったかをメモする。
