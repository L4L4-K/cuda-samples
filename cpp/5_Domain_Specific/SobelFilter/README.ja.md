# SobelFilter - Sobel Filter - Japanese Learning Guide
...
Original README headings: `SobelFilter - Sobel Filter`, `Description`, `Key Concepts`, `Supported SM Architectures`, `Supported OSes`, `Supported CPU Architecture`, `CUDA APIs involved`, `[CUDA Runtime API](http://docs.nvidia.com/cuda/cuda-runtime-api/index.html)`, `Dependencies needed to build/run`, `Prerequisites`, `References (for more details)`

> **日本語**
> `cpp/5_Domain_Specific/SobelFilter` は `C++/CUDA` の CUDA sample です。英語 README を正本として残し、この guide では実装を読む順番と CUDA の学習ポイントを日本語で補足します。
>
> **学習メモ**
...
English anchor: read `SobelFilter` as a focused example of the CUDA concepts used in `cpp/5_Domain_Specific/SobelFilter`.
> **日本語**
> この sample の目的は、`SobelFilter` の小さな実装を通して CUDA Graphs, Shared Memory, Streams And Events, Synchronization And Atomics, Memory を具体的に追うことです。
>
> **学習メモ**
> 最初に `SobelFilter.cpp, SobelFilter_kernels.cu, SobelFilter_kernels.h` を読み、setup、GPU work、sync、validation、cleanup の境界を source 内で対応させます。
...
- `SobelFilter.cpp`: Host-side setup, API calls, validation, and cleanup.
- `SobelFilter_kernels.cu`: CUDA source containing kernels, Runtime API calls, or device-side helper code.
- `SobelFilter_kernels.h`: Host/device declarations, helper types, constants, or library wrappers.
...
- `data/teapot.pgm`: Input, reference, generated-data description, or documentation used by the sample.
...
- Open `SobelFilter.cpp` first and locate the host-side setup or Python entry point.
...
```bash
cmake -S . -B build
cmake --build build --target SobelFilter
ctest --test-dir build -R SobelFilter
```
