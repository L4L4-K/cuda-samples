# radixSortThrust - CUDA Radix Sort (Thrust Library) - Japanese Learning Guide
...
Original README headings: `radixSortThrust - CUDA Radix Sort (Thrust Library)`, `Description`, `Key Concepts`, `Supported SM Architectures`, `Supported OSes`, `Supported CPU Architecture`, `CUDA APIs involved`, `[CUDA Runtime API](http://docs.nvidia.com/cuda/cuda-runtime-api/index.html)`, `Prerequisites`, `References (for more details)`

> **日本語**
> `cpp/2_Concepts_and_Techniques/radixSortThrust` は `C++/CUDA` の CUDA sample です。英語 README を正本として残し、この guide では実装を読む順番と CUDA の学習ポイントを日本語で補足します。
>
> **学習メモ**
...
English anchor: read `radixSortThrust` as a focused example of the CUDA concepts used in `cpp/2_Concepts_and_Techniques/radixSortThrust`.
> **日本語**
> この sample の目的は、`radixSortThrust` の小さな実装を通して CUDA Libraries, Streams And Events, Performance, Memory, Execution Model を具体的に追うことです。
>
> **学習メモ**
> 最初に `radixSortThrust.cu` を読み、setup、GPU work、sync、validation、cleanup の境界を source 内で対応させます。
...
- `doc/readme.txt`: Input, reference, generated-data description, or documentation used by the sample.
- `radixSortThrust.cu`: CUDA source containing kernels, Runtime API calls, or device-side helper code.
...
- Open `radixSortThrust.cu` first and locate the host-side setup or Python entry point.
...
- `radixSortThrust.cu`: focus on `thrust::host_vector`, `cudaEventDestroy`, `thrust::device_vector`, `cudaEvent_t`, `cudaEventCreate`.
...
Source: cpp/2_Concepts_and_Techniques/radixSortThrust/CMakeLists.txt:1-37
```cmake
...
> JP: この抜粋は `cpp/2_Concepts_and_Techniques/radixSortThrust/CMakeLists.txt` の実コードです。setup、allocation、transfer、GPU work、sync、validation、cleanup のどの境界を示すかを、行番号と一緒に確認します。

### `radixSortThrust.cu`

Source: cpp/2_Concepts_and_Techniques/radixSortThrust/radixSortThrust.cu:29-47
```cuda
#include <algorithm>
...
> JP: この抜粋は `cpp/2_Concepts_and_Techniques/radixSortThrust/radixSortThrust.cu` の実コードです。setup、allocation、transfer、GPU work、sync、validation、cleanup のどの境界を示すかを、行番号と一緒に確認します。

Source: cpp/2_Concepts_and_Techniques/radixSortThrust/radixSortThrust.cu:154-182
```cuda
        d_keys = h_keys;
...
Source: cpp/2_Concepts_and_Techniques/radixSortThrust/radixSortThrust.cu:202-221
```cuda
    }
...
| `thrust::copy` | host/device 間の転送、初期化、または visibility を作る API です。方向と Async の順序を確認します。 |
| `cudaDeviceProp` | この sample の中心 API/概念です。入力、所有権、同期、検証との関係を確認します。 |
| `thrust::default_random_engine` | CUDA library call です。handle/descriptor/workspace と data layout を確認します。 |
| `thrust::uniform_real_distribution` | CUDA library call です。handle/descriptor/workspace と data layout を確認します。 |
...
```bash
cmake -S . -B build
cmake --build build --target radixSortThrust
ctest --test-dir build -R radixSortThrust
```
...
- `cudaEventDestroy` の直前と直後で、どの memory/resource が有効になったかをメモする。
