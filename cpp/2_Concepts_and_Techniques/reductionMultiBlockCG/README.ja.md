# reductionMultiBlockCG - Reduction using MultiBlock Cooperative Groups - Japanese Learning Guide
...
Original README headings: `reductionMultiBlockCG - Reduction using MultiBlock Cooperative Groups`, `Description`, `Key Concepts`, `Supported SM Architectures`, `Supported OSes`, `Supported CPU Architecture`, `CUDA APIs involved`, `[CUDA Runtime API](http://docs.nvidia.com/cuda/cuda-runtime-api/index.html)`, `Dependencies needed to build/run`, `Prerequisites`, `References (for more details)`

> **日本語**
> `cpp/2_Concepts_and_Techniques/reductionMultiBlockCG` は `C++/CUDA` の CUDA sample です。英語 README を正本として残し、この guide では実装を読む順番と CUDA の学習ポイントを日本語で補足します。
>
> **学習メモ**
...
English anchor: read `reductionMultiBlockCG` as a focused example of the CUDA concepts used in `cpp/2_Concepts_and_Techniques/reductionMultiBlockCG`.
> **日本語**
> この sample の目的は、`reductionMultiBlockCG` の小さな実装を通して Cooperative Groups, Shared Memory, Streams And Events, Synchronization And Atomics, Performance を具体的に追うことです。
>
> **学習メモ**
> 最初に `reductionMultiBlockCG.cu` を読み、setup、GPU work、sync、validation、cleanup の境界を source 内で対応させます。
...
- `reductionMultiBlockCG.cu`: CUDA source containing kernels, Runtime API calls, or device-side helper code.
...
- Open `reductionMultiBlockCG.cu` first and locate the host-side setup or Python entry point.
...
- `reductionMultiBlockCG.cu`: focus on `launch`, `cudaMemcpy`, `CUDA`, `cudaMalloc`, `blockDim`.
...
```bash
cmake -S . -B build
cmake --build build --target reductionMultiBlockCG
ctest --test-dir build -R reductionMultiBlockCG
```
