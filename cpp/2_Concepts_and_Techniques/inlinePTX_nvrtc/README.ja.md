# inlinePTX_nvrtc - Using Inline PTX with libNVRTC - Japanese Learning Guide
...
English anchor: read `inlinePTX_nvrtc` as a focused example of the CUDA concepts used in `cpp/2_Concepts_and_Techniques/inlinePTX_nvrtc`.
> **日本語**
> この sample の目的は、`inlinePTX_nvrtc` の小さな実装を通して Runtime, Driver, And NVRTC, Multi-GPU, P2P, And IPC, Performance, Memory, Kernel Launch And Indexing を具体的に追うことです。
>
> **学習メモ**
> 最初に `inlinePTX.cpp, inlinePTX_kernel.cu` を読み、setup、GPU work、sync、validation、cleanup の境界を source 内で対応させます。
...
- `inlinePTX.cpp`: Host-side setup, API calls, validation, and cleanup.
- `inlinePTX_kernel.cu`: CUDA source containing kernels, Runtime API calls, or device-side helper code.
...
- Open `inlinePTX.cpp` first and locate the host-side setup or Python entry point.
...
- `inlinePTX.cpp`: focus on `cudaBlockSize`, `CUDA`, `cudaGridSize`, `cuMemAlloc`, `cuLaunchKernel`.
- `inlinePTX_kernel.cu`: focus on `launch`, `blockIdx`, `blockDim`, `threadIdx`.
...
```bash
cmake -S . -B build
cmake --build build --target inlinePTX_nvrtc
ctest --test-dir build -R inlinePTX_nvrtc
```
...
- `cudaBlockSize` の直前と直後で、どの memory/resource が有効になったかをメモする。
