# threadMigration - CUDA Context Thread Management - Japanese Learning Guide
...
Original README headings: `threadMigration - CUDA Context Thread Management`, `Description`, `Key Concepts`, `Supported SM Architectures`, `Supported OSes`, `Supported CPU Architecture`, `CUDA APIs involved`, `[CUDA Driver API](http://docs.nvidia.com/cuda/cuda-driver-api/index.html)`, `Prerequisites`, `References (for more details)`

> **日本語**
> `cpp/2_Concepts_and_Techniques/threadMigration` は `C++/CUDA` の CUDA sample です。英語 README を正本として残し、この guide では実装を読む順番と CUDA の学習ポイントを日本語で補足します。
>
> **学習メモ**
...
English anchor: read `threadMigration` as a focused example of the CUDA concepts used in `cpp/2_Concepts_and_Techniques/threadMigration`.
> **日本語**
> この sample の目的は、`threadMigration` の小さな実装を通して Runtime, Driver, And NVRTC, Memory, Kernel Launch And Indexing, Execution Model, Debugging, Profiling, And Testing を具体的に追うことです。
>
> **学習メモ**
> 最初に `threadMigration.cpp, threadMigration_kernel.cu` を読み、setup、GPU work、sync、validation、cleanup の境界を source 内で対応させます。
...
- `threadMigration.cpp`: Host-side setup, API calls, validation, and cleanup.
- `threadMigration_kernel.cu`: CUDA source containing kernels, Runtime API calls, or device-side helper code.
...
- Open `threadMigration.cpp` first and locate the host-side setup or Python entry point.
...
```bash
cmake -S . -B build
cmake --build build --target threadMigration
ctest --test-dir build -R threadMigration
```
