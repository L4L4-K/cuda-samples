# alignedTypes - Aligned Types - Japanese Learning Guide
...
Original README headings: `alignedTypes - Aligned Types`, `Description`, `Key Concepts`, `Supported SM Architectures`, `Supported OSes`, `Supported CPU Architecture`, `CUDA APIs involved`, `[CUDA Runtime API](http://docs.nvidia.com/cuda/cuda-runtime-api/index.html)`, `Prerequisites`, `References (for more details)`

> **日本語**
> `cpp/6_Performance/alignedTypes` は `C++/CUDA` の CUDA sample です。英語 README を正本として残し、この guide では実装を読む順番と CUDA の学習ポイントを日本語で補足します。
>
> **学習メモ**
...
English anchor: read `alignedTypes` as a focused example of the CUDA concepts used in `cpp/6_Performance/alignedTypes`.
> **日本語**
> この sample の目的は、`alignedTypes` の小さな実装を通して Performance, Memory, Kernel Launch And Indexing, Execution Model, Debugging, Profiling, And Testing を具体的に追うことです。
>
> **学習メモ**
> 最初に `alignedTypes.cu` を読み、setup、GPU work、sync、validation、cleanup の境界を source 内で対応させます。
...
- `alignedTypes.cu`: CUDA source containing kernels, Runtime API calls, or device-side helper code.
- `doc/alignedTypes.txt`: Input, reference, generated-data description, or documentation used by the sample.
...
- Open `alignedTypes.cu` first and locate the host-side setup or Python entry point.
...
- `alignedTypes.cu`: focus on `CUDA`, `blockDim`, `cudaDeviceSynchronize`, `cudaMalloc`, `cudaFree`.
...
Source: cpp/6_Performance/alignedTypes/alignedTypes.cu:39-57
```cuda
#include <math.h>
#include <stdio.h>
#include <stdlib.h>
...
> JP: この抜粋は `cpp/6_Performance/alignedTypes/alignedTypes.cu` の実コードです。setup、allocation、transfer、GPU work、sync、validation、cleanup のどの境界を示すかを、行番号と一緒に確認します。

Source: cpp/6_Performance/alignedTypes/alignedTypes.cu:131-162
```cuda
...
Source: cpp/6_Performance/alignedTypes/alignedTypes.cu:182-201
```cuda
{
...
Source: cpp/6_Performance/alignedTypes/alignedTypes.cu:214-233
```cuda
    printf(flag ? "\tTEST OK\n" : "\tTEST FAILURE\n");
...
```bash
cmake -S . -B build
cmake --build build --target alignedTypes
ctest --test-dir build -R alignedTypes
```
